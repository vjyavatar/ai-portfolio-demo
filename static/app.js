...
const TRADES_EMAILS=['x@yp.com'];
const PICKS_EMAILS=['x@yp.com'];
let _pulseData=null;
let _pulseFetching=false,_pulseLoaded=false;

// ═══ LIVE MACRO INTELLIGENCE — populates homepage cards from market-pulse ═══
function renderMacroIntel(data){
if(!data)return;
const wrap=document.getElementById('macroIntel');
const cards=document.getElementById('macroCards');
const upcoming=document.getElementById('macroUpcoming');
if(!wrap||!cards)return;

let html='';
// Commodity/Market snapshot
const snap=data.global_snapshot||{};
const order=['S&P 500','Crude Oil','Gold','US Dollar','USD/INR','Silver'];
order.forEach(name=>{
const s=snap[name];
if(!s)return;
const up=s.change_pct>=0;
const c=up?'#0eb24e':'#cd004b';
const bg=up?'rgba(14,178,78,.04)':'rgba(205,0,75,.04)';
const bc=up?'rgba(14,178,78,.12)':'rgba(205,0,75,.12)';
const arrow=up?'&#9650;':'&#9660;';
const icon=name.includes('Oil')?'&#128738;':name.includes('Gold')?'&#129351;':name.includes('Silver')?'&#129352;':name.includes('S&P')?'&#128200;':name.includes('Dollar')?'&#128178;':'&#127470;&#127475;';
...
// Events (war, tariffs, macro)
if(data.events&&data.events.length>0){
data.events.forEach(evt=>{
const sC=evt.severity==='HIGH'?'#cd004b':evt.severity==='MEDIUM'?'#fcb711':'#1a56db';
const iC=evt.impact==='BULLISH'?'#0eb24e':evt.impact==='BEARISH'?'#cd004b':'#fcb711';
const iBg=evt.impact==='BULLISH'?'rgba(14,178,78,.04)':evt.impact==='BEARISH'?'rgba(205,0,75,.04)':'rgba(252,183,17,.04)';
html+=`<div style="padding:12px 14px;border-radius:8px;background:${iBg};border:1px solid ${sC}22;border-left:3px solid ${sC}">
<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
<span style="font-size:7px;padding:2px 6px;border-radius:100px;background:${sC};color:#fff;font-weight:700;letter-spacing:.5px">${evt.severity}</span>
<span style="font-size:11px;font-weight:700;color:var(--text)">${evt.headline}</span>
</div>
<div style="font-size:10px;color:var(--text2);line-height:1.6">${evt.detail||''}</div>
${evt.action?'<div style="font-size:10px;color:'+iC+';font-weight:600;margin-top:4px">'+evt.action+'</div>':''}
</div>`;
});
}
...
// Upcoming events
if(data.upcoming&&data.upcoming.length>0&&upcoming){
let uh='<div style="padding:12px 16px;border-radius:8px;background:var(--bg2);border:1px solid var(--border)">';
uh+='<div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:8px">&#128197; Upcoming Events (next 45 days)</div>';
uh+='<div style="display:flex;flex-wrap:wrap;gap:6px">';
data.upcoming.slice(0,12).forEach(u=>{
const uC=u.impact==='HIGH'?'#cd004b':'#fcb711';
uh+=`<span style="font-size:10px;padding:4px 10px;border-radius:6px;background:rgba(0,47,108,.04);border:1px solid var(--border);color:var(--text2)"><strong style="color:${uC}">${u.event}</strong> &middot; ${u.date} <span style="color:var(--text3)">(${u.days}d)</span></span>`;
});
uh+='</div></div>';
upcoming.innerHTML=uh;
}
...
if(!hist.length)return '<div style="margin-top:10px;padding:10px;border-radius:6px;background:var(--bg2);text-align:center;font-size:10px;color:var(--text3)">5-day history will build up automatically over the next few trading days.</div>';
let t='<div style="margin-top:12px"><div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:6px;display:flex;align-items:center;gap:6px">&#128197; FII/DII — Last '+hist.length+' Trading Days</div>';
t+='<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:10px;font-family:var(--mono)">';
t+='<tr style="background:rgba(0,47,108,.04)"><th style="padding:6px 8px;text-align:left;color:var(--text3);font-weight:700;border-bottom:2px solid var(--border)">Date</th><th style="padding:6px 8px;text-align:right;color:#1a56db;border-bottom:2px solid var(--border)">FII Buy</th><th style="padding:6px 8px;text-align:right;color:#1a56db;border-bottom:2px solid var(--border)">FII Sell</th><th style="padding:6px 8px;text-align:right;color:#1a56db;font-weight:800;border-bottom:2px solid var(--border)">FII Net</th><th style="padding:6px 8px;text-align:right;color:var(--purple);border-bottom:2px solid var(--border)">DII Buy</th><th style="padding:6px 8px;text-align:right;color:var(--purple);border-bottom:2px solid var(--border)">DII Sell</th><th style="padding:6px 8px;text-align:right;color:var(--purple);font-weight:800;border-bottom:2px solid var(--border)">DII Net</th><th style="padding:6px 8px;text-align:right;font-weight:800;border-bottom:2px solid var(--border)">Combined</th></tr>';
const fmt=(v)=>{const a=Math.abs(v);return (v>=0?'+':'-')+'₹'+(a>=10000?(a/1000).toFixed(0)+'K':a.toLocaleString('en-IN'))+'Cr';};
hist.slice().reverse().forEach((day,i)=>{
const fC=day.fii_net>=0?'var(--green)':'var(--red)';
const dC=day.dii_net>=0?'var(--green)':'var(--red)';
const cC=day.combined>=0?'var(--green)':'var(--red)';
const bg=i%2===0?'transparent':'rgba(0,47,108,.015)';
t+='<tr style="background:'+bg+';border-bottom:1px solid var(--border)">';
t+='<td style="padding:5px 8px;font-weight:600;color:var(--text)">'+(day.date||'')+'</td>';
t+='<td style="padding:5px 8px;text-align:right;color:var(--text2)">₹'+(day.fii_buy||0).toLocaleString('en-IN')+'</td>';
t+='<td style="padding:5px 8px;text-align:right;color:var(--text2)">₹'+(day.fii_sell||0).toLocaleString('en-IN')+'</td>';
t+='<td style="padding:5px 8px;text-align:right;font-weight:800;color:'+fC+'">'+fmt(day.fii_net)+'</td>';
t+='<td style="padding:5px 8px;text-align:right;color:var(--text2)">₹'+(day.dii_buy||0).toLocaleString('en-IN')+'</td>';
t+='<td style="padding:5px 8px;text-align:right;color:var(--text2)">₹'+(day.dii_sell||0).toLocaleString('en-IN')+'</td>';
t+='<td style="padding:5px 8px;text-align:right;font-weight:800;color:'+dC+'">'+fmt(day.dii_net)+'</td>';
t+='<td style="padding:5px 8px;text-align:right;font-weight:800;color:'+cC+'">'+fmt(day.combined)+'</td>';
t+='</tr>';
});
// Totals row
if(hist.length>1){
const totFii=hist.reduce((s,d)=>s+(d.fii_net||0),0);
const totDii=hist.reduce((s,d)=>s+(d.dii_net||0),0);
const totComb=totFii+totDii;
t+='<tr style="background:rgba(0,47,108,.06);border-top:2px solid var(--border)">';
t+='<td style="padding:6px 8px;font-weight:800;color:var(--text)">'+hist.length+'-Day Total</td>';
t+='<td colspan="2"></td>';
t+='<td style="padding:6px 8px;text-align:right;font-weight:800;font-size:11px;color:'+(totFii>=0?'var(--green)':'var(--red)')+'">'+fmt(totFii)+'</td>';
t+='<td colspan="2"></td>';
t+='<td style="padding:6px 8px;text-align:right;font-weight:800;font-size:11px;color:'+(totDii>=0?'var(--green)':'var(--red)')+'">'+fmt(totDii)+'</td>';
t+='<td style="padding:6px 8px;text-align:right;font-weight:800;font-size:11px;color:'+(totComb>=0?'var(--green)':'var(--red)')+'">'+fmt(totComb)+'</td>';
t+='</tr>';
}
t+='</table></div>';
// Trend insight
if(hist.length>=3){
const last3Fii=hist.slice(-3).reduce((s,d)=>s+(d.fii_net||0),0);
const last3Dii=hist.slice(-3).reduce((s,d)=>s+(d.dii_net||0),0);
const trend=last3Fii>3000&&last3Dii>0?'🟢 Strong institutional support — both FII and DII net buyers over 3 days. Bullish for markets.':last3Fii<-3000&&last3Dii>3000?'🟡 FII selling absorbed by DII — markets may range-bound. Watch for FII reversal.':last3Fii<-3000&&last3Dii<0?'🔴 Both FII and DII selling — weak institutional support. Caution advised.':last3Fii>1000?'🟢 FII buying trend — positive for large-caps and banking.':last3Fii<-1000?'🟡 FII outflow trend — IT exporters may benefit from weaker rupee.':'⚪ Mixed signals — no clear institutional trend.';
t+='<div style="margin-top:8px;padding:8px 12px;border-radius:6px;background:rgba(37,99,235,.03);border-left:3px solid var(--blue);font-size:10px;color:var(--text2)"><strong>3-Day Trend:</strong> '+trend+'</div>';
}
t+='</div>';
return t;
})()}
...
el.innerHTML=`
<div class="sc" style="border-left:3px solid ${signalC};margin-top:12px">
...
<div class="sc" data-tab="quick" data-subtab="overview" style="border-left:3px solid #1a56db;background:linear-gradient(135deg,rgba(0,47,108,.02),rgba(101,97,172,.02));padding:0">
<div style="padding:14px 16px">
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;text-align:center">
<div style="padding:10px;border-radius:8px;background:var(--surface);border:1px solid var(--border);cursor:pointer" onclick="switchSubTab('overview')">
<div style="font-size:16px;margin-bottom:2px">&#9889;</div>
<div style="font-size:10px;font-weight:700;color:var(--green)">Should I Buy?</div>
<div style="font-size:8px;color:var(--text3)">Verdict + Key Numbers</div>
</div>
<div style="padding:10px;border-radius:8px;background:var(--surface);border:1px solid var(--border);cursor:pointer" onclick="switchSubTab('valuation')">
<div style="font-size:16px;margin-bottom:2px">&#128176;</div>
<div style="font-size:10px;font-weight:700;color:#1a56db">Is It Overpriced?</div>
<div style="font-size:8px;color:var(--text3)">Fair Value + Peers</div>
</div>
<div style="padding:10px;border-radius:8px;background:var(--surface);border:1px solid var(--border);cursor:pointer" onclick="switchSubTab('technical')">
<div style="font-size:16px;margin-bottom:2px">&#127919;</div>
<div style="font-size:10px;font-weight:700;color:#6561ac">When to Buy/Sell?</div>
<div style="font-size:8px;color:var(--text3)">Entry/Exit + Charts</div>
</div>
<div style="padding:10px;border-radius:8px;background:var(--surface);border:1px solid var(--border);cursor:pointer" onclick="switchSubTab('activity')">
<div style="font-size:16px;margin-bottom:2px">&#128100;</div>
<div style="font-size:10px;font-weight:700;color:var(--blue)">Who is Trading?</div>
<div style="font-size:8px;color:var(--text3)">Insider + FII/DII</div>
</div>
<div style="padding:10px;border-radius:8px;background:var(--surface);border:1px solid var(--border);cursor:pointer" onclick="switchSubTab('risk')">
<div style="font-size:16px;margin-bottom:2px">&#9888;</div>
<div style="font-size:10px;font-weight:700;color:#cd004b">What Could Go Wrong?</div>
<div style="font-size:8px;color:var(--text3)">Debt + Volatility</div>
</div>
</div>
</div></div>

<!-- ABOUT COMPANY — SEO-rich content -->
<div class="sc" data-tab="quick" data-subtab="overview" style="border-left:3px solid #1a56db">
<div class="sh"><div class="si" style="background:rgba(0,47,108,.08)">🏢</div><div class="st" style="color:#1a56db">About ${d.company_name||d.ticker}</div></div>
<div class="sbody">
<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px">
${d.sector&&d.sector!=='N/A'&&d.sector!=='Unknown'?'<span style="font-size:10px;padding:3px 10px;border-radius:100px;background:rgba(0,47,108,.06);color:#1a56db;font-weight:600;border:1px solid rgba(0,47,108,.12)">'+d.sector+'</span>':''}
${d.industry&&d.industry!=='N/A'&&d.industry!=='Unknown'?'<span style="font-size:10px;padding:3px 10px;border-radius:100px;background:rgba(13,148,136,.06);color:#0d9488;font-weight:600;border:1px solid rgba(13,148,136,.12)">'+d.industry+'</span>':''}
${d.exchange&&d.exchange!=='N/A'?'<span style="font-size:10px;padding:3px 10px;border-radius:100px;background:rgba(139,92,246,.06);color:#7c3aed;font-weight:600;border:1px solid rgba(139,92,246,.12)">'+d.exchange+'</span>':''}
</div>
<div style="display:flex;align-items:center;gap:12px;padding:10px 14px;border-radius:8px;background:var(--bg2);border:1px solid var(--border);margin-bottom:12px;flex-wrap:wrap">
...
const desc=d.company_description||'';
if(desc)return desc.length>500?desc.substring(0,500)+'...':desc;
const parts=[];
parts.push((d.company_name||d.ticker)+(d.sector&&d.sector!=='N/A'&&d.sector!=='Unknown'?' is a '+d.sector+' company':' is a publicly traded company'));
if(d.industry&&d.industry!=='N/A'&&d.industry!=='Unknown')parts[0]+=' operating in the '+d.industry+' industry';
parts[0]+='.';
if(d.market_cap&&d.market_cap>0){const ms=d.market_cap>=1e12?curr+(d.market_cap/1e12).toFixed(1)+' trillion':d.market_cap>=1e9?curr+(d.market_cap/1e9).toFixed(1)+' billion':curr+(d.market_cap/1e6).toFixed(0)+' million';parts.push('The company has a market capitalization of approximately '+ms+'.')}
if(d.employees&&d.employees!=='N/A'&&d.employees>0)parts.push('It employs approximately '+(d.employees>=1e3?(d.employees/1e3).toFixed(0)+',000+':d.employees.toLocaleString())+' people.');
if(pe>0)parts.push('The stock currently trades at a P/E ratio of '+pe.toFixed(1)+(pe<15?', suggesting attractive valuation.':pe<25?'.':', reflecting growth expectations.'));
const dy=parseFloat(d.dividend_yield)||0;
if(dy>0)parts.push('It offers a dividend yield of '+dy.toFixed(2)+'%.');
return parts.join(' ');
})()}</p>
${d.website?'<div style="margin-top:8px"><a href="'+d.website+'" target="_blank" rel="noopener" style="font-size:11px;color:#1a56db;text-decoration:none;font-weight:600">&#128279; '+d.website.replace('https://','').replace('http://','').replace(/\/+$/,'')+'</a></div>':''}
</div></div>

<!-- VERDICT (always open) -->
<div class="sc" data-tab="quick" data-subtab="overview" style="border-left:3px solid var(--green)"><div class="sh"><div class="si" style="background:rgba(16,185,129,.1)">&#9678;</div><div class="st" style="color:var(--green)" id="sec-verdict">Verdict</div></div>
...
<div class="sc" data-tab="quick" data-subtab="overview" data-conditional="1" style="border-left:3px solid var(--blue);display:none" id="fiiDiiCard">
...
<!-- DECISION POINTS -->
<div class="sc" data-tab="quick" data-subtab="overview" style="border-left:3px solid var(--amber)"><div class="sh"><div class="si" style="background:rgba(245,158,11,.1)">&#9651;</div><div class="st" style="color:var(--amber)" id="sec-5things">Key Numbers at a Glance</div></div>
<div class="sbody">
...
${d.market_cap&&d.market_cap>1e6?'<div style="padding:10px;border-radius:8px;text-align:center;background:rgba(0,47,108,.04);border:1px solid rgba(0,47,108,.12)"><div style="font-size:8px;text-transform:uppercase;letter-spacing:.5px;color:var(--text3);font-weight:700">Market Cap</div><div style="font-family:var(--mono);font-size:18px;font-weight:800;color:var(--text)">'+(d.market_cap>=1e12?curr+(d.market_cap/1e12).toFixed(2)+'T':d.market_cap>=1e9?curr+(d.market_cap/1e9).toFixed(2)+'B':curr+(d.market_cap/1e6).toFixed(0)+'M')+'</div><div style="font-size:9px;color:var(--text3)">'+d.currency+'</div></div>':''}
${d.week52_high&&d.week52_low&&parseFloat(d.week52_high)>0?'<div style="padding:10px;border-radius:8px;text-align:center;background:rgba(0,47,108,.04);border:1px solid rgba(0,47,108,.12)"><div style="font-size:8px;text-transform:uppercase;letter-spacing:.5px;color:var(--text3);font-weight:700">52W Range</div><div style="font-family:var(--mono);font-size:14px;font-weight:800;color:var(--text)">'+curr+d.week52_low+' — '+curr+d.week52_high+'</div><div style="font-size:9px;color:var(--text3)">Position: '+((price/parseFloat(d.week52_high))*100).toFixed(0)+'% of high</div></div>':''}
...
<div style="font-size:9px;color:var(--text3);margin-bottom:2px">VOLATILITY (Beta)</div>
<div style="font-size:13px;font-weight:700;color:${parseFloat(d.beta)>1.3?'var(--red)':parseFloat(d.beta)<0.8?'var(--green)':'var(--text)'}">${parseFloat(d.beta).toFixed(2)} <span style="font-size:10px;color:var(--text3)">${parseFloat(d.beta)>1.5?'Very volatile':parseFloat(d.beta)>1.2?'Above average':parseFloat(d.beta)<0.7?'Defensive':'Normal'}</span></div>
</div>`:''}
...
<div style="font-size:9px;color:var(--text3);margin-bottom:4px">52W PRICE POSITION</div>
<div style="display:flex;align-items:center;gap:8px">
<span style="font-size:9px;color:var(--red);font-family:var(--mono)">${curr}${d.week52_low}</span>
<div style="flex:1;height:8px;border-radius:4px;background:linear-gradient(90deg,var(--red),var(--amber),var(--green));position:relative">
<div style="position:absolute;top:-3px;left:${Math.min(95,Math.max(5,((price-parseFloat(d.week52_low))/(parseFloat(d.week52_high)-parseFloat(d.week52_low)))*100))}%;width:14px;height:14px;border-radius:50%;background:#fff;border:2px solid var(--text);box-shadow:0 1px 4px rgba(0,0,0,.2);transform:translateX(-50%)"></div>
</div>
<span style="font-size:9px;color:var(--green);font-family:var(--mono)">${curr}${d.week52_high}</span>
</div>
<div style="font-size:9px;color:var(--text3);text-align:center;margin-top:2px">${((price/parseFloat(d.week52_high))*100).toFixed(0)}% of 52W high ${(d.beta&&d.beta!=='N/A')?'· Beta: '+parseFloat(d.beta).toFixed(2):''}</div>
</div>`:''}
...
<!-- STOCK YTD + YEARLY PERFORMANCE -->
<div class="sc" data-tab="quick" data-subtab="overview" style="border-left:3px solid var(--cyan)">
<div class="sh"><div class="si" style="background:rgba(6,182,212,.1)">&#128200;</div><div class="st" style="color:var(--cyan)">Historical Performance — YTD &amp; Yearly Returns</div></div>
<div class="sbody">
<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
<!-- YTD Card -->
<div style="flex:1;min-width:120px;padding:14px;border-radius:10px;text-align:center;background:${d.ytd_return!==undefined&&d.ytd_return!==null?(d.ytd_return>=0?'rgba(14,178,78,.06)':'rgba(205,0,75,.06)'):'var(--bg2)'};border:1px solid ${d.ytd_return!==undefined&&d.ytd_return!==null?(d.ytd_return>=0?'rgba(14,178,78,.15)':'rgba(205,0,75,.15)'):'var(--border)'}">
<div style="font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">YTD ${new Date().getFullYear()}</div>
<div style="font-size:24px;font-weight:800;color:${d.ytd_return>=0?'var(--green)':'var(--red)'};font-family:var(--mono)">${d.ytd_return!==undefined&&d.ytd_return!==null?(d.ytd_return>=0?'+':'')+d.ytd_return+'%':'N/A'}</div>
<div style="font-size:9px;color:var(--text3);margin-top:2px">${d.ytd_return>20?'Outstanding year so far &#128293;':d.ytd_return>10?'Strong performance &#128170;':d.ytd_return>0?'Positive but moderate':d.ytd_return>-10?'Minor decline':d.ytd_return<=-10?'Significant drop &#9888;':'Year-to-date return'}</div>
</div>
${(()=>{
const yr=d.yearly_returns||{};
const keys=Object.keys(yr).sort().reverse();
if(!keys.length)return '<div style="flex:1;min-width:200px;padding:14px;text-align:center;color:var(--text3);font-size:11px">Yearly data not available</div>';
let cards='';
keys.forEach(y=>{
const v=yr[y];
const up=v>=0;
cards+='<div style="flex:1;min-width:80px;padding:10px;border-radius:8px;text-align:center;background:'+(up?'rgba(14,178,78,.04)':'rgba(205,0,75,.04)')+';border:1px solid '+(up?'rgba(14,178,78,.1)':'rgba(205,0,75,.1)') +'">';
cards+='<div style="font-size:9px;color:var(--text3)">'+y+'</div>';
cards+='<div style="font-size:16px;font-weight:800;color:'+(up?'var(--green)':'var(--red)')+';font-family:var(--mono)">'+(up?'+':'')+v+'%</div>';
cards+='</div>';
});
return cards;
})()}
...
const vals=keys.map(k=>yr[k]);
const avg=(vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1);
const best=Math.max(...vals);const worst=Math.min(...vals);
const bestYr=keys[vals.indexOf(best)];const worstYr=keys[vals.indexOf(worst)];
return '<div style="padding:10px 14px;border-radius:8px;background:rgba(6,182,212,.03);border-left:3px solid var(--cyan);font-size:10px;color:var(--text2);line-height:1.7">'
+'<strong style="color:var(--cyan)">5-Year Summary:</strong> '
+'Avg annual return <strong>'+avg+'%</strong> · '
+'Best year <strong style="color:var(--green)">'+bestYr+' ('+best+'%)</strong> · '
+'Worst year <strong style="color:var(--red)">'+worstYr+' ('+worst+'%)</strong> · '
+(parseFloat(avg)>15?'Outperforming market — strong compounder &#128293;':parseFloat(avg)>8?'Solid performer — above FD returns':'Below average — check fundamentals')
+'</div>';
})()}
</div></div>

<!-- VALUATION TABLE -->
<div class="sc" data-tab="quick" data-subtab="valuation" style="border-left:3px solid var(--blue)"><div class="sh"><div class="si" style="background:rgba(0,120,212,.12)">&#9678;</div><div class="st" style="color:var(--blue)" id="sec-valuation">Is This Stock Overpriced?</div></div>
<div class="sbody">
<p style="font-size:12px;color:var(--text3);margin-bottom:14px">Every number decoded. No jargon:</p>
<table class="dt"><tr><th>Metric</th><th>Value</th><th>Signal</th><th>Verdict</th></tr>
...
<td style="font-size:12px;line-height:1.7">${pe<15?'Bargain. ✅ Buy zone.':pe<25?'Fair price.':d.pe_ratio+'x. Pricey. Needs growth.'}</td></tr>`:''}
...
<td style="font-size:12px;line-height:1.7">${parseFloat(d.pb_ratio)<3?d.pb_ratio+'x book. Fair.':d.pb_ratio+'x book. Premium.'}</td></tr>`:''}
...
<td style="font-size:12px;line-height:1.7">${parseFloat(d.dividend_yield)>3?'Good income ✅':parseFloat(d.dividend_yield)>1?'Modest income':'Growth-focused'}</td></tr>`:''}
</table>
...
<div style="font-size:11px;color:var(--text2);line-height:1.8">
${pe>0&&pe<15?'<strong style="color:var(--green)">🟢 UNDERVALUED.</strong> Cheap relative to earnings. Potential buying opportunity.'
:pe>=15&&pe<25?'<strong style="color:var(--blue)">🟡 FAIRLY VALUED.</strong> Reasonable price. Good entry if you believe in the story.'
:pe>=25&&pe<40?'<strong style="color:var(--amber)">🟠 PREMIUM.</strong> Market expects high growth. If earnings miss, price drops fast.'
:pe>=40?'<strong style="color:var(--red)">🔴 EXPENSIVE.</strong> High risk of correction. Only for aggressive growth investors.'
...
<div class="sc" data-tab="quick" data-subtab="valuation" style="border-left:3px solid var(--teal)"><div class="sh"><div class="si" style="background:rgba(20,184,166,.1)">⚔</div><div class="st" style="color:var(--teal)" id="sec-peers">Peer Comparison</div></div>
...
<div class="sc" data-tab="quick" data-subtab="technical" style="border-left:3px solid var(--purple)"><div class="sh"><div class="si" style="background:rgba(139,92,246,.1)">⬡</div><div class="st" style="color:var(--purple)" id="sec-entry">Entry &amp; Exit Levels</div></div>
<div class="sbody">
...
<!-- RISK ASSESSMENT -->
<div class="sc" data-tab="quick" data-subtab="risk" style="border-left:3px solid var(--red)"><div class="sh"><div class="si" style="background:rgba(239,68,68,.1)">&#9888;</div><div class="st" style="color:var(--red)" id="sec-risk">What Could Go Wrong?</div></div>
<div class="sbody">
...
if(de>0)rows+=`<tr><td>Debt Level</td><td style="font-family:var(--mono);font-weight:600">${d.debt_to_equity}</td><td><span class="sig ${de>1.5?'r':de>0.8?'c':'g'}">${de>1.5?'High':de>0.8?'Moderate':'Low'}</span></td><td style="font-size:12px">${de>1.5?'High debt ⚠':de>0.8?'OK':'Low debt ✅'}</td></tr>`;
if(cr>0)rows+=`<tr><td>Liquidity</td><td style="font-family:var(--mono);font-weight:600">${d.current_ratio}</td><td><span class="sig ${cr>1.5?'g':cr>1?'c':'r'}">${cr>1.5?'Strong':cr>1?'Adequate':'Weak'}</span></td><td style="font-size:12px">${cr>1.5?'Strong cash ✅':cr>1?'Tight':'Weak cash ⚠'}</td></tr>`;
rows+=`<tr><td>Volatility</td><td style="font-family:var(--mono);font-weight:600">${bt.toFixed(2)}</td><td><span class="sig ${bt>1.5?'r':bt<0.8?'g':'n'}">${bt>1.5?'High':bt<0.8?'Low':'Average'}</span></td><td style="font-size:12px">${bt>1.5?'Wild swings ⚠':bt<0.8?'Stable ✅':'Normal'}</td></tr>`;
...
<div style="font-size:11px;color:var(--text2);line-height:1.8">
${(()=>{const de=parseFloat(d.debt_to_equity)||0,cr=parseFloat(d.current_ratio)||0,bt=parseFloat(d.beta)||1;
if(de>1.5&&cr>0&&cr<1)return '<strong style="color:var(--red)">🔴 HIGH RISK</strong> — heavy debt + weak cash. One bad quarter = serious trouble.';
if(de>1.5)return '<strong style="color:var(--amber)">🟠 ELEVATED DEBT</strong> — vulnerable if rates rise or business slows.';
if(cr>0&&cr<1)return '<strong style="color:var(--amber)">🟡 TIGHT CASH</strong> — may struggle with short-term bills. Watch quarterly results.';
return '<strong style="color:var(--green)">🟢 MANAGEABLE</strong> — reasonable debt, enough cash. No red flags.';})()}
${(()=>{const bt=parseFloat(d.beta)||1;return bt>1.5?' Expect bigger swings than market.':'';})()}
</div>
</div>
</div></div>

<!-- CHARTS (Summary — hidden until sub-tab click) -->
<div class="sc" data-tab="quick" data-subtab="technical" style="border-left:3px solid var(--cyan)" id="chartsCard">
...
<div class="sc" data-tab="analysis" data-asub="report" style="border-left:3px solid var(--blue)"><div class="sh"><div class="si" style="background:rgba(0,120,212,.12)">🎓</div><div class="st" style="color:var(--blue)" id="sec-verdictai" data-asub="report">AI Verdict Breakdown — Green, Yellow & Red</div></div>
<div class="sbody"><div id="aiAnalysisFormatted"></div></div></div>

<!-- Full AI Report -->
<div class="sc" data-tab="analysis" data-asub="report" style="border-left:3px solid var(--blue)"><div class="sh"><div class="si" style="background:rgba(0,120,212,.12)">&#9889;</div><div class="st" style="color:var(--blue)" id="sec-ai" data-asub="report">AI Analyst Report — Full Depth</div></div>
<div class="sbody">
...
<div class="sc" data-tab="analysis" data-asub="earnings" style="border-left:3px solid var(--purple)"><div class="sh"><div class="si" style="background:rgba(16,185,129,.1)">&#128200;</div><div class="st" style="color:var(--green)" id="sec-fund">Latest Quarter &#8212; Are They Actually Growing?</div></div>
<div class="sbody">
...
<!-- Management Tone -->
<div class="sc" data-tab="analysis" data-asub="management" style="border-left:3px solid var(--purple)"><div class="sh"><div class="si" style="background:rgba(168,85,247,.1)">&#127908;</div><div class="st" style="color:var(--purple)" id="sec-mgmt">Management Tone — Read Between the Lines</div></div>
<div class="sbody">
<div id="mgmtContent"><div class="ic pu" style="text-align:center"><p style="color:var(--text3)">&#9203; Analyzing management tone...</p></div></div>
</div></div>

<!-- FUND HOLDINGS -->
<div class="sc" data-tab="analysis" data-asub="management" style="border-left:3px solid var(--purple)"><div class="sh"><div class="si" style="background:rgba(0,120,212,.12)">&#127974;</div><div class="st" style="color:var(--blue)" id="sec-funds">Top Fund &amp; Institutional Holdings</div></div>
<div class="sbody">
...
<!-- ═══ TAB: FORECAST ═══ -->
<div class="sc" data-tab="analysis" data-asub="outlook" style="border-left:3px solid var(--cyan)"><div class="sh"><div class="si" style="background:rgba(6,182,212,.1)">&#128302;</div><div class="st" style="color:var(--cyan)" id="sec-next">What's Next — Catalysts &amp; Timeline</div></div>
<div class="sbody">
...
// ═══ ANALYSIS SUB-TAB SYSTEM ═══
window._activeAnalysisSubTab='report';
function switchAnalysisSubTab(sub){
window._activeAnalysisSubTab=sub;
document.querySelectorAll('[data-tab="analysis"][data-asub]').forEach(el=>{el.style.display='none'});
document.querySelectorAll('[data-tab="analysis"][data-asub="'+sub+'"]').forEach(el=>{el.style.display='block'});
document.querySelectorAll('#analysisSubTabs .stab').forEach(b=>b.classList.remove('active'));
var btns=document.querySelectorAll('#analysisSubTabs .stab');
const map={report:0,earnings:1,management:2,outlook:3};
if(btns[map[sub]])btns[map[sub]].classList.add('active');
const first=document.querySelector('[data-tab="analysis"][data-asub="'+sub+'"]:not([style*="display:none"])');
if(first)first.scrollIntoView({behavior:'instant',block:'start'});
}

// ═══ SUMMARY SUB-TAB SYSTEM ═══
window._activeSubTab='overview';
function switchSubTab(sub){
window._activeSubTab=sub;
// Hide all quick sections that have a subtab
document.querySelectorAll('[data-tab="quick"][data-subtab]').forEach(el=>{
  el.style.display='none';
});
// Show sections matching this subtab
document.querySelectorAll('[data-tab="quick"][data-subtab="'+sub+'"]').forEach(el=>{
  if(el.dataset.conditional==='1'){
    // FII/DII — only show if it has content
    if(el.querySelector('#fiiDiiBody')&&el.querySelector('#fiiDiiBody').innerHTML.trim())el.style.display='block';
  } else {
    if(el.innerHTML.trim())el.style.display='block';
  }
});
// Update active button
document.querySelectorAll('#summarySubTabs .stab').forEach(b=>b.classList.remove('active'));
const btnMap={overview:'stOverview',valuation:'stValuation',technical:'stTechnical',activity:'stActivity',risk:'stRisk'};
const btn=document.getElementById(btnMap[sub]);
if(btn)btn.classList.add('active');
// Scroll to top of content
const firstVisible=document.querySelector('[data-tab="quick"][data-subtab="'+sub+'"]:not([style*="display:none"]):not([style*="display: none"])');
if(firstVisible)firstVisible.scrollIntoView({behavior:'instant',block:'start'});
...
function switchTabGroup(group) {
  ...
  if (!window._isPremiumUser) {
    ...
    ['smarttrades','journal','aiassist','valreport','backtest','stockintel'].forEach(function(t){
  ...
  if (subNav) {
    if (tabs.length <= 1) {
      ...
      var h = '';
      var _tips={trades:'Live 5-engine algo signals with entry, SL, targets',smarttrades:'AI daily trade ideas for indices & stocks',stockintel:'Complete 13-section BUY/SELL/HOLD decision engine',scanner:'Heatmap, screener & options flow',valreport:'PE valuation, crash scenarios & geo risks',backtest:'Test strategies on historical data',journal:'Log trades, track P&L & behavior',aiassist:'Ask about any stock in plain English',analysis:'AI deep research report',dcf:'Intrinsic value calculator',equity:'Equity research with peers',compare:'Side-by-side stock comparison',quick:'Summary, charts & key metrics',indices:'Top performers & YTD rankings',daily:'Daily market analysis',finance:'SIP calculator & tax tools',education:'Learn options, Greeks & strategies'};
      tabs.forEach(function(tab, i) {
        var isActive = (i === 0);
        var tip=_tips[tab]||labels[i];
        h += '<button onclick="switchTab(\''+tab+'\')" class="sub-tab-btn'+(isActive?' active':'')+'" title="'+tip+'" style="padding:4px 12px;border-radius:20px;font-size:9px;font-weight:700;border:1px solid '+(isActive?'var(--blue)':'var(--border)')+';background:'+(isActive?'var(--blue)':'transparent')+';color:'+(isActive?'#fff':'var(--text3)')+';cursor:pointer;font-family:Sora,sans-serif;transition:all .2s;white-space:nowrap">'+labels[i]+'</button>';
      });
      subNav.innerHTML = h;
    }
  }
...
// For quick tab, sections with subtab are handled by switchSubTab
...
document.querySelectorAll('#tabContentArea > [data-tab="'+tab+'"]').forEach(s=>{
if(tab==='quick' && s.dataset.subtab) return; // handled by switchSubTab
if(s.innerHTML.trim())s.style.display=s.classList.contains('mgrid')?'grid':'block';
});
// For quick tab, activate sub-tab system
if(tab==='quick'){
var subBar=document.getElementById('summarySubTabs');
if(subBar)subBar.style.display='flex';
switchSubTab(window._activeSubTab||'overview');
}
if(tab==='analysis'){
var aBar=document.getElementById('analysisSubTabs');
...
if(dw){let wh='';for(let r=0;r<6;r++)for(let c=0;c<4;c++){const x=c*25+Math.random()*5;const y=r*18+Math.random()*5;wh+=`<span style="position:absolute;left:${x}%;top:${y}%;font-size:10px;font-weight:800;color:#1a56db;transform:rotate(-35deg);white-space:nowrap;letter-spacing:2px;font-family:'Sora',sans-serif">CELESYS.AI</span>`}dw.innerHTML=wh}
...
<table class="dt"><thead><tr><th style="width:25%">Factor</th><th style="width:22%">Status</th><th>Verdict</th></tr></thead>
...
// ═══ FCF ESTIMATION (STABILIZED) ═══
// Use multiple methods and average to prevent single-source volatility
const reportedFCF=fcf>0?fcf:0;
const ocfBased=ocf>0?ocf*0.75:0;
const revBased=rev>0?rev*Math.max(pm,5)/100*0.6:0;
const ebitdaBased=ebitda>0?ebitda*0.5:0;
const mcapBased=mcap>0?mcap*0.035:0;
// Average all available methods for stability
const fcfMethods=[reportedFCF,ocfBased,revBased,ebitdaBased].filter(v=>v>0);
const baseFCF=fcfMethods.length>0?(fcfMethods.reduce((a,b)=>a+b,0)/fcfMethods.length):(mcapBased||1);
...
// ═══ GROWTH RATE ESTIMATION (STABILIZED) ═══
const histGrowth=Math.max(Math.min(((rg||0)+(eg||0)+(eqg||0))/3/100,0.4),-0.1);
const sustainGrowth=Math.max(Math.min(roe/100*(1-payout/100),0.3),0);
const analystGrowth=fpe>0&&eps>0?Math.max(Math.min((epsF/eps-1),0.3),-0.1):null;
const impliedGrowth=Math.max(Math.min(pe>5&&costEquity>0?(1/pe):0.08,0.2),0.02);
// Weighted blend — analyst estimate gets highest weight if available
const weights=analystGrowth!==null?[0.25,0.2,0.35,0.2]:[0.4,0.3,0,0.3];
const growthInputs=[histGrowth||0.05,sustainGrowth||0.05,analystGrowth||0,impliedGrowth||0.05];
const blendedGrowth=Math.max(Math.min(growthInputs.reduce((s,v,i)=>s+v*weights[i],0),0.3),-0.03)||0.06;
...
var ytdC=s.ytd>20?'#047857':s.ytd>0?'#10b981':s.ytd===0?'var(--text3)':s.ytd>-20?'#ef4444':'#991b1b';
...
try{
// html2pdf loaded statically via script tag
if(!window.html2pdf){
...
// Get ticker name from current report header
const ticker=(document.getElementById('tickerTag')?.textContent||'Report').trim();
const company=(document.getElementById('companyName')?.textContent||'').trim();
const filename=(ticker+'_'+(company||'Celesys')+'_Report').replace(/[^a-zA-Z0-9_]/g,'_')+'.pdf';

// Clone only VISIBLE sections to avoid rendering hidden tabs
const wrapper=document.createElement('div');
wrapper.style.cssText='padding:16px;font-family:DM Sans,sans-serif;max-width:800px';

// Add header
wrapper.innerHTML='<div style="text-align:center;margin-bottom:16px;padding-bottom:12px;border-bottom:2px solid #1a56db"><div style="font-size:22px;font-weight:800;color:#1a56db;font-family:Sora,sans-serif">'+ticker+' — '+(company||'Stock Analysis')+'</div><div style="font-size:11px;color:#666;margin-top:4px">Celesys AI Report · '+(document.getElementById('timestamp')?.textContent||new Date().toLocaleDateString())+'</div></div>';

// Gather visible .sc sections from current tab
const visible=document.querySelectorAll('#tabContentArea .sc[data-tab]');
let count=0;
visible.forEach(sc=>{
if(sc.offsetParent!==null&&sc.style.display!=='none'){
const clone=sc.cloneNode(true);
// Remove interactive elements
clone.querySelectorAll('button,input,canvas,details').forEach(e=>{
if(e.tagName==='CANVAS'){
try{const img=document.createElement('img');img.src=sc.querySelector('canvas')?.toDataURL('image/png')||'';img.style.cssText='width:100%;max-height:200px';e.replaceWith(img);}catch(ex){e.remove();}
}else if(e.tagName==='DETAILS'){e.setAttribute('open','');}
else{e.remove();}
});
...
if(count===0){throw new Error('No visible content. Switch to a tab with data first.');}

// Add footer
wrapper.innerHTML+='<div style="margin-top:20px;padding-top:10px;border-top:1px solid #ddd;text-align:center;font-size:9px;color:#999">Generated by Celesys AI · celesys.ai · For educational purposes only. Not financial advice.</div>';
...
alert('PDF generation failed: '+e.message+'\n\nTip: Use Ctrl+P and Save as PDF as an alternative.');
}finally{
if(btn){btn.innerHTML='&#128196;';btn.disabled=false;}
}
}
...
const ticker=(document.getElementById('tickerTag')?.textContent||'Stock').trim();
const company=(document.getElementById('companyName')?.textContent||'').trim();
const price=window._stockData?.current_price||'';
const curr=window._stockCurr||'$';
const url=window.location.href;
const text=`Check out this detailed AI analysis of ${ticker}${company?' ('+company+')':''} ${price?curr+price:''} on Celesys AI! ${url}`;
...
async function fetchPerformanceLeaderboard(){
const el=document.getElementById('perfLeaderboard');
if(!el)return;
try{
const r=await fetch('/api/performance-leaderboard');
const data=await r.json();
if(!data.success){el.innerHTML='<div style="text-align:center;color:var(--text3);font-size:11px">Performance data unavailable</div>';return}
renderPerfLeaderboard(el,data);
}catch(e){el.innerHTML='<div style="text-align:center;color:var(--text3);font-size:11px">Could not load performance data</div>';}
}

function renderPerfLeaderboard(el,data){
const C=(v)=>v>=0?'var(--green)':'var(--red)';
const F=(v)=>v!==null&&v!==undefined?(v>=0?'+':'')+v.toFixed(1)+'%':'N/A';

const rankTbl=(title,icon,best,worst)=>{
if(!best||!best.length)return '';
let t='<div style="margin-bottom:10px"><div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:4px">'+icon+' '+title+'</div>';
t+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">';
t+='<div style="border-radius:6px;border:1px solid rgba(14,178,78,.12);overflow:hidden"><div style="padding:5px 8px;background:rgba(14,178,78,.06);font-size:9px;font-weight:700;color:var(--green)">&#128293; Best</div>';
best.forEach((s,i)=>{t+='<div style="padding:3px 8px;font-size:10px;display:flex;justify-content:space-between;border-bottom:1px solid var(--border)"><span>'+s.name+'</span><span style="font-weight:700;color:var(--green);font-family:var(--mono)">'+F(s.return)+'</span></div>'});
t+='</div>';
t+='<div style="border-radius:6px;border:1px solid rgba(205,0,75,.12);overflow:hidden"><div style="padding:5px 8px;background:rgba(205,0,75,.06);font-size:9px;font-weight:700;color:var(--red)">&#128308; Worst</div>';
(worst||[]).forEach((s,i)=>{t+='<div style="padding:3px 8px;font-size:10px;display:flex;justify-content:space-between;border-bottom:1px solid var(--border)"><span>'+s.name+'</span><span style="font-weight:700;color:var(--red);font-family:var(--mono)">'+F(s.return)+'</span></div>'});
t+='</div></div></div>';
return t;
};

const barChart=(items,yearKey)=>{
let b='';
const sorted=[...items].sort((a,b)=>{
const av=yearKey==='ytd'?(a.ytd||0):(a.yearly[yearKey]||0);
const bv=yearKey==='ytd'?(b.ytd||0):(b.yearly[yearKey]||0);
return bv-av;
});
sorted.forEach(idx=>{
const v=yearKey==='ytd'?(idx.ytd||0):(idx.yearly[yearKey]||0);
const w=Math.min(100,Math.abs(v)*2);
b+='<div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">';
b+='<div style="width:90px;font-size:9px;font-weight:600;color:var(--text);text-align:right;flex-shrink:0;overflow:hidden;white-space:nowrap;text-overflow:ellipsis">'+idx.name+'</div>';
b+='<div style="flex:1;height:16px;background:var(--bg3);border-radius:3px;overflow:hidden"><div style="width:'+w+'%;height:100%;background:'+C(v)+';border-radius:3px;opacity:.7"></div></div>';
b+='<div style="width:50px;font-size:10px;font-weight:700;color:'+C(v)+';font-family:var(--mono);text-align:right">'+F(v)+'</div></div>';
});
return b;
};

// ═══ BUILD COUNTRY SECTION ═══
const buildCountry=(country,flag,sym,label)=>{
if(!country)return '';
let h='';

// YTD
h+='<div style="padding:14px;border-radius:10px;background:linear-gradient(135deg,rgba(0,47,108,.02),rgba(6,182,212,.01));border:1px solid var(--border);margin-bottom:12px">';
h+='<div style="font-size:13px;font-weight:800;color:var(--text);margin-bottom:10px">'+flag+' '+label+' — YTD '+data.current_year+'</div>';

if(country.indices.length)h+='<div style="margin-bottom:10px"><div style="font-size:10px;font-weight:700;color:var(--text3);margin-bottom:4px">INDICES</div>'+barChart(country.indices,'ytd')+'</div>';
if(country.ytd){
h+=rankTbl('Top & Bottom ETFs YTD','&#128230;',country.ytd.etfs?.best,country.ytd.etfs?.worst);
h+=rankTbl('Top & Bottom Stocks YTD','&#128202;',country.ytd.stocks?.best,country.ytd.stocks?.worst);
}
h+='</div>';

// Yearly
h+='<div style="padding:14px;border-radius:10px;background:var(--bg2);border:1px solid var(--border)">';
h+='<div style="font-size:13px;font-weight:800;color:var(--text);margin-bottom:8px">'+flag+' '+label+' — Year-by-Year (5Y)</div>';

const years=data.years||[];
const geoKey=label.includes('India')?'in':'us';
h+='<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:10px">';
years.forEach((yr,i)=>{
h+='<button class="stab'+(i===0?' active':'')+'" onclick="showYearPerf'+geoKey+'(this,\''+yr+'\')" style="padding:3px 12px;font-size:10px">'+yr+'</button>';
});
h+='</div>';

years.forEach((yr,i)=>{
const yrData=country.yearly[yr]||{};
h+='<div class="yr-panel-'+geoKey+'" data-yr="'+yr+'" style="'+(i>0?'display:none':'')+'">';
if(country.indices.length)h+='<div style="margin-bottom:8px"><div style="font-size:10px;font-weight:700;color:var(--text3);margin-bottom:4px">INDICES IN '+yr+'</div>'+barChart(country.indices,yr)+'</div>';
if(yrData.stocks)h+=rankTbl('Stocks in '+yr,'&#128202;',yrData.stocks.best,yrData.stocks.worst);
if(yrData.etfs)h+=rankTbl('ETFs in '+yr,'&#128230;',yrData.etfs.best,yrData.etfs.worst);
h+='</div>';
});
h+='</div>';
...
// ═══ RENDER WITH COUNTRY TABS ═══
let h='';
h+='<div style="display:flex;gap:6px;margin-bottom:12px">';
h+='<button class="stab active" onclick="switchPerfGeo(this,\'india\')" style="padding:6px 18px;font-size:11px">&#127470;&#127475; India</button>';
h+='<button class="stab" onclick="switchPerfGeo(this,\'usa\')" style="padding:6px 18px;font-size:11px">&#127482;&#127480; USA</button>';
h+='</div>';

h+='<div id="perfIndia">'+buildCountry(data.india,'&#127470;&#127475;','₹','India')+'</div>';
h+='<div id="perfUsa" style="display:none">'+buildCountry(data.usa,'&#127482;&#127480;','$','USA')+'</div>';

el.innerHTML=h;
}

// Country + Year switchers
window.switchPerfGeo=function(btn,geo){
document.getElementById('perfIndia').style.display=geo==='india'?'':'none';
document.getElementById('perfUsa').style.display=geo==='usa'?'':'none';
btn.parentElement.querySelectorAll('.stab').forEach(b=>b.classList.remove('active'));
btn.classList.add('active');
};
window.showYearPerfin=function(btn,yr){
document.querySelectorAll('.yr-panel-in').forEach(p=>p.style.display='none');
var panel=document.querySelector('.yr-panel-in[data-yr="'+yr+'"]');
if(panel)panel.style.display='block';
btn.parentElement.querySelectorAll('.stab').forEach(b=>b.classList.remove('active'));
btn.classList.add('active');
};
window.showYearPerfus=function(btn,yr){
document.querySelectorAll('.yr-panel-us').forEach(p=>p.style.display='none');
var panel=document.querySelector('.yr-panel-us[data-yr="'+yr+'"]');
if(panel)panel.style.display='block';
btn.parentElement.querySelectorAll('.stab').forEach(b=>b.classList.remove('active'));
btn.classList.add('active');
};
...
if(!trades.length&&signal==='N/A'){
el.innerHTML='<div style="padding:14px;border-radius:8px;background:rgba(139,92,246,.04);border:1px solid rgba(139,92,246,.12);text-align:center"><div style="font-size:12px;font-weight:700;color:var(--purple);margin-bottom:4px">&#128100; Insider Activity</div><div style="font-size:11px;color:var(--text2)">No recent insider transactions found. This could mean insiders are holding steady — neither buying nor selling. Check SEC/NSE filings for latest updates.</div></div>';
...
if(!nextE||nextE==='N/A'){
el.innerHTML='<div style="padding:14px;border-radius:8px;background:rgba(245,158,11,.04);border:1px solid rgba(245,158,11,.12);text-align:center"><div style="font-size:12px;font-weight:700;color:var(--amber);margin-bottom:4px">&#128197; Next Earnings Date</div><div style="font-size:11px;color:var(--text2)">Earnings date not yet announced. Check the company\'s investor relations page for updates.</div></div>';
el.style.display='block';return;}
el.style.display='block';
...
// ═══ HIGH-CONVICTION TRADE CONFLUENCE ENGINE ═══
...
window._algoAutoInterval=null;
window._algoLastTrend=null;
function toggleAlgoAuto(sym){
if(window._algoAutoInterval){
clearInterval(window._algoAutoInterval);
window._algoAutoInterval=null;
var btn=document.getElementById('algoAutoBtn');
if(btn){btn.textContent='▶ Auto 2min';btn.style.background='rgba(14,178,78,.1)';btn.style.color='#0eb24e';}
var dot=document.getElementById('algoLiveDot');
if(dot)dot.style.background='#94a3b8';
}else{
window._algoAutoInterval=setInterval(function(){
algoSelect(sym,null);
},120000); // 2 minutes
var btn=document.getElementById('algoAutoBtn');
if(btn){btn.textContent='⏹ Stop Auto';btn.style.background='rgba(239,68,68,.1)';btn.style.color='#ef4444';}
}
}
function _checkTrendChange(d){
if(!d.trend)return;
var newTrend=d.trend.label;
if(window._algoLastTrend&&window._algoLastTrend!==newTrend){
// TREND CHANGED — show notification
var alertDiv=document.createElement('div');
alertDiv.style.cssText='position:fixed;top:60px;right:20px;z-index:9999;padding:16px 24px;border-radius:12px;background:#1a56db;color:#fff;font-family:Sora,sans-serif;font-size:13px;font-weight:700;box-shadow:0 8px 32px rgba(0,0,0,.3);animation:fi .3s ease;max-width:350px';
alertDiv.innerHTML='<div style="font-size:16px;margin-bottom:4px">&#128680; TREND CHANGE</div><div>'+d.symbol+': <span style="text-decoration:line-through;opacity:.6">'+window._algoLastTrend+'</span> &#8594; <span style="color:'+(d.trend.pct>=0?'#4ade80':'#f87171')+'">'+newTrend+'</span></div><div style="font-size:10px;opacity:.7;margin-top:4px">'+d.timestamp+'</div>';
document.body.appendChild(alertDiv);
setTimeout(function(){alertDiv.style.opacity='0';alertDiv.style.transition='opacity .5s';setTimeout(function(){alertDiv.remove()},600)},8000);
...
['algoRegIN','scanRegIN'].forEach(function(id){var b=document.getElementById(id);if(b){b.style.background=reg==='IN'?'#1a56db':'var(--bg2)';b.style.color=reg==='IN'?'#fff':'var(--text3)'}});
['algoRegUS','scanRegUS'].forEach(function(id){var b=document.getElementById(id);if(b){b.style.background=reg==='US'?'#1a56db':'var(--bg2)';b.style.color=reg==='US'?'#fff':'var(--text3)'}});
...
window._tradesToday=(window._tradesToday||0);

// Game Plan header
var h='<div id="expiryBanner"></div>';
h+='<div style="padding:14px;border-radius:12px;background:linear-gradient(135deg,rgba(0,47,108,.06),rgba(6,182,212,.04));border:1px solid var(--border);margin-bottom:14px">';
h+='<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:10px">';
h+='<div style="font-size:14px;font-weight:900;color:var(--text)">&#128197; TODAY\'S GAME PLAN</div>';
h+='<div style="font-size:9px;color:var(--text3)">'+new Date().toLocaleDateString('en-IN',{weekday:'long',day:'numeric',month:'short',year:'numeric'})+'</div></div>';
h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-bottom:10px" id="gamePlanMetrics">';
h+='<div style="padding:8px;border-radius:8px;background:var(--surface);border:1px solid var(--border);text-align:center"><div style="font-size:7px;color:var(--text3);font-weight:700">TRADES REMAINING</div><div style="font-size:20px;font-weight:900;color:var(--blue)">'+(3-window._tradesToday)+'/3</div><div style="font-size:7px;color:var(--text3)">Max 3 trades/day</div></div>';
h+='<div style="padding:8px;border-radius:8px;background:var(--surface);border:1px solid var(--border);text-align:center"><div style="font-size:7px;color:var(--text3);font-weight:700">MARKET MODE</div><div style="font-size:14px;font-weight:800;color:var(--cyan)" id="gpMarketMode">Analyzing...</div></div>';
h+='<div style="padding:8px;border-radius:8px;background:var(--surface);border:1px solid var(--border);text-align:center"><div style="font-size:7px;color:var(--text3);font-weight:700">STRATEGY TODAY</div><div style="font-size:10px;font-weight:800;color:var(--amber)" id="gpStrategy">Loading...</div></div>';
h+='<div style="padding:8px;border-radius:8px;background:var(--surface);border:1px solid var(--border);text-align:center"><div style="font-size:7px;color:var(--text3);font-weight:700">DISCIPLINE</div><div style="font-size:10px;font-weight:700;color:#10b981" id="gpDiscipline">&#9989; On track</div></div>';
h+='</div>';
h+='<div style="font-size:9px;color:var(--text2);line-height:1.5" id="gpTip">&#128161; Loading today\'s key levels and expected moves...</div>';
h+='</div>';

// Trade cards container
h+='<div style="font-size:12px;font-weight:800;color:var(--text);margin-bottom:10px">&#127919; HIGH-CONFIDENCE SETUPS — Click any card for full analysis</div>';
h+='<div id="tradeCards" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:10px">';
symbols.forEach(function(s){
h+='<div id="tc_'+s+'" style="padding:14px;border-radius:12px;border:1px solid var(--border);background:var(--surface);min-height:120px;cursor:pointer" onclick="algoSelect(\''+s+'\',null)">';
h+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><div style="font-size:14px;font-weight:900;color:var(--text)">'+s+'</div>';
h+='<div style="width:14px;height:14px;border:2px solid var(--blue);border-top-color:transparent;border-radius:50%;animation:spin .5s linear infinite"></div><span style="font-size:10px;color:var(--text3)">Analyzing...</span></div>';
h+='</div>';
});
h+='</div>';
el.innerHTML=h;
...
if(card)card.outerHTML=_buildTradeCard(d,S,reg);
// Re-highlight if this was the active card
if(window._activeAlgoSym===s){var rc=document.getElementById('tc_'+s);if(rc){rc.style.border='2px solid #1a56db';rc.style.boxShadow='0 0 24px rgba(26,86,219,.2),inset 0 0 0 1px rgba(26,86,219,.1)';rc.style.background='rgba(26,86,219,.04)';rc.style.transform='scale(1.02)'}}
// Update game plan from first result
if(idx===0)_updateGamePlan(d,S);
// Expiry banner
if(d.isExpiry||d.allExpiryToday?.length>0){
var ban=document.getElementById('expiryBanner');
if(ban&&!ban.dataset.set){ban.dataset.set='1';
ban.innerHTML='<div style="padding:10px 16px;border-radius:10px;background:linear-gradient(135deg,rgba(239,68,68,.1),rgba(245,158,11,.06));border:2px solid rgba(239,68,68,.25);margin-bottom:12px;display:flex;align-items:center;gap:10px"><span style="font-size:24px;animation:pulse 1.5s infinite">&#128293;</span><div><div style="font-size:13px;font-weight:800;color:#ef4444">EXPIRY DAY — '+(d.allExpiryToday||[]).join(', ')+'</div><div style="font-size:10px;color:var(--text2)">Gamma risk HIGH. Tighter SL mandatory. Exit by 2:30 PM. Like driving fast — exciting but one wrong turn is costly.</div></div></div>';}
}
}).catch(function(){});
});
...
function _updateGamePlan(d,S){
var vi=d.volIntelligence||{};var oi=d.options_intel||{};
var mode=document.getElementById('gpMarketMode');
var strat=document.getElementById('gpStrategy');
var tip=document.getElementById('gpTip');
var disc=document.getElementById('gpDiscipline');
if(mode){
var regime=vi.regime||'NORMAL';
var modeText=regime.includes('SELL')?'HIGH VOLATILITY':regime.includes('BUY')?'LOW VOLATILITY':regime.includes('ELEV')?'ELEVATED':'NORMAL';
var modeC=regime.includes('SELL')?'#ef4444':regime.includes('BUY')?'#10b981':'var(--cyan)';
mode.style.color=modeC;mode.textContent=modeText;
}
if(strat){
var stratText=vi.regime==='HIGH_IV_SELL'?'Sell Premium (Iron Condors)':vi.regime==='LOW_IV_BUY'?'Buy Options (Directional)':vi.regime==='ELEVATED'?'Spreads (Defined Risk)':'Follow Trend Direction';
strat.textContent=stratText;
}
if(tip){
var t=d.technicals||{};var em=oi.expected_move||0;
var tipText='Key Levels: Support '+S+(t.sma20||0).toLocaleString('en-IN')+' | Resistance '+S+(t.w52h||0).toLocaleString('en-IN');
if(em>0)tipText+=' | Expected Move: ±'+S+em.toLocaleString('en-IN');
tipText+=' | IV Rank: '+(oi.iv_rank||50)+'%';
if(oi.iv_rank>=60)tipText+=' (Options expensive — favor selling)';
else if(oi.iv_rank<=30)tipText+=' (Options cheap — favor buying)';
tip.innerHTML='&#128161; '+tipText;
}
if(disc){
if(window._tradesToday>=3)disc.innerHTML='<span style="color:#ef4444">&#9888; STOP — 3 trade limit reached today</span>';
else disc.innerHTML='&#9989; '+window._tradesToday+'/3 trades taken. Discipline intact.';
}
}

function _buildTradeCard(d,S,reg){
var tr=d.trade||{};var tc=d.tradeConfidence||{};var sc=d.scalp||{};var vi=d.volIntelligence||{};
var sig=d.signal||'N/A';var dir=d.direction||'NEUTRAL';
var win=tc.estimatedWin||50;var grade=tc.grade||'C';
var isWait=sig==='HOLD / WAIT'||sig==='AVOID';
var dC=dir==='BULLISH'?'#047857':dir==='BEARISH'?'#ef4444':'#f59e0b';
var winC=win>=65?'#047857':win>=50?'#f59e0b':'#ef4444';
var gradeC=grade.startsWith('A')?'#047857':grade.startsWith('B')?'#3b82f6':'#f59e0b';
var lightEmoji=win>=65?'&#128994;':win>=50?'&#128993;':'&#128308;';
var isUS=reg==='US';

var h='<div style="padding:0;border-radius:12px;border:2px solid '+winC+'30;background:var(--surface);overflow:hidden;cursor:pointer;transition:transform .15s" onmouseover="this.style.transform=\'scale(1.01)\'" onmouseout="this.style.transform=\'scale(1)\'" onclick="algoSelect(\''+d.symbol+'\',null)">';

// Header bar
h+='<div style="padding:10px 14px;background:'+dC+'08;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">';
h+='<div style="display:flex;align-items:center;gap:8px">';
h+='<div style="width:32px;height:32px;border-radius:50%;background:'+gradeC+';display:flex;align-items:center;justify-content:center"><span style="font-size:13px;font-weight:900;color:#fff">'+grade+'</span></div>';
h+='<div><div style="font-size:15px;font-weight:900;color:var(--text)">'+d.symbol+(d.isExpiry?' <span style="font-size:7px;padding:1px 5px;border-radius:3px;background:#ef444420;color:#ef4444;font-weight:800">EXPIRY</span>':'')+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">'+(d.instrument?.ex||'NFO')+' &middot; '+S+(d.spot||0).toLocaleString("en-IN")+'</div></div></div>';
h+='<div style="text-align:right"><div style="font-size:9px;font-weight:800;padding:3px 10px;border-radius:4px;background:'+dC+'15;color:'+dC+'">'+dir+'</div>';
h+='<div style="font-size:18px;font-weight:900;color:'+winC+';font-family:var(--mono)">'+win+'%</div></div>';
h+='</div>';

if(tr&&!isWait){
// Main trade action
h+='<div style="padding:10px 14px;border-bottom:1px solid var(--border)">';
h+='<div style="font-size:12px;font-weight:800;color:'+dC+';margin-bottom:6px">'+lightEmoji+' '+tr.action+'</div>';
// Entry / SL / Target grid
h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:6px">';
h+='<div style="text-align:center;padding:6px;border-radius:6px;background:var(--bg2)"><div style="font-size:7px;color:var(--text3);font-weight:700">ENTRY</div><div style="font-size:14px;font-weight:900;font-family:var(--mono);color:var(--text)">'+S+(tr.premEntry||0).toFixed(1)+'</div></div>';
h+='<div style="text-align:center;padding:6px;border-radius:6px;background:rgba(239,68,68,.04)"><div style="font-size:7px;color:#ef4444;font-weight:700">STOP LOSS</div><div style="font-size:14px;font-weight:900;font-family:var(--mono);color:#ef4444">'+S+(tr.premSL||0).toFixed(1)+'</div></div>';
h+='<div style="text-align:center;padding:6px;border-radius:6px;background:rgba(16,185,129,.04)"><div style="font-size:7px;color:#10b981;font-weight:700">TARGET</div><div style="font-size:14px;font-weight:900;font-family:var(--mono);color:#10b981">'+S+(tr.premT2||0).toFixed(1)+'</div></div>';
h+='</div>';
// Risk pre-calculated
var riskAmt=Math.abs((tr.premEntry||0)-(tr.premSL||0));
var rewardAmt=Math.abs((tr.premT2||0)-(tr.premEntry||0));
h+='<div style="display:flex;gap:8px;font-size:9px;flex-wrap:wrap">';
h+='<span style="padding:3px 8px;border-radius:4px;background:#ef444410;color:#ef4444;font-weight:700">Risk: '+S+riskAmt.toFixed(1)+'/unit</span>';
h+='<span style="padding:3px 8px;border-radius:4px;background:#10b98110;color:#10b981;font-weight:700">Reward: '+S+rewardAmt.toFixed(1)+'/unit</span>';
h+='<span style="padding:3px 8px;border-radius:4px;background:var(--bg2);color:var(--text);font-weight:700">R:R '+(tr.rrRatio||'N/A')+'</span>';
h+='</div>';
h+='</div>';

// Volatility + Strategy one-liner
h+='<div style="padding:8px 14px;border-bottom:1px solid var(--border);font-size:9px;color:var(--text2);line-height:1.5">';
var regime=vi.regime||'NORMAL';
if(regime==='HIGH_IV_SELL')h+='<strong style="color:#ef4444">&#127777; Options expensive</strong> — favor selling premium. Iron Condor or Credit Spread better than naked buy.';
else if(regime==='LOW_IV_BUY')h+='<strong style="color:#10b981">&#127777; Options cheap</strong> — good time to buy. Straddle before events or directional trade.';
else h+='<strong style="color:var(--cyan)">&#127777; Normal volatility</strong> — go with directional view. '+vi.action||'';
h+='</div>';

// Layman one-liner
h+='<div style="padding:8px 14px;font-size:9px;color:var(--text3);line-height:1.5">';
h+='&#128161; '+(dir==='BULLISH'?'Betting '+d.symbol+' goes UP. Pay '+S+(tr.premEntry||0).toFixed(0)+', exit at '+S+(tr.premSL||0).toFixed(0)+' if wrong, cash out at '+S+(tr.premT2||0).toFixed(0)+' if right.':'Betting '+d.symbol+' goes DOWN. Pay '+S+(tr.premEntry||0).toFixed(0)+', exit at '+S+(tr.premSL||0).toFixed(0)+' if wrong, cash out at '+S+(tr.premT2||0).toFixed(0)+' if right.');
h+='</div>';
}else{
// No trade
h+='<div style="padding:14px;text-align:center">';
h+='<div style="font-size:12px;font-weight:700;color:#f59e0b;margin-bottom:4px">&#9208; NO HIGH-CONFIDENCE TRADE</div>';
h+='<div style="font-size:9px;color:var(--text3)">Mixed signals — like weather forecast saying 50% rain. Wait for clarity.</div>';
if(sc&&sc.direction&&sc.direction!=='NEUTRAL'&&sc.confidence>=40){
h+='<div style="margin-top:8px;padding:8px;border-radius:6px;background:rgba(6,182,212,.06);border:1px solid rgba(6,182,212,.2)">';
h+='<div style="font-size:10px;font-weight:800;color:var(--cyan)">&#9889; SCALP ALTERNATIVE ('+sc.confidence+'%)</div>';
h+='<div style="font-size:9px;color:var(--text2)">'+sc.action+' — Entry: '+S+sc.entry+' | SL: '+S+sc.sl+' | Tgt: '+S+sc.t1+'</div>';
h+='<div style="font-size:8px;color:var(--text3);margin-top:2px">Quick 5-30 min trade based on 5m+15m price action</div>';
h+='</div>';
}
h+='</div>';
}
...
var isUS=(d.instrument&&d.instrument.region||'')==='US';
var S=isUS?'$':'&#8377;';
var tr=d.trade;var tc=d.tradeConfidence||{};var sc=d.scalp||{};
var sig=d.signal||'N/A';var dir=d.direction||'NEUTRAL';
var dC=dir==='BULLISH'?'#059669':dir==='BEARISH'?'#dc2626':'#d97706';
var isWait=sig==='HOLD / WAIT'||sig==='AVOID';
var win=tc.estimatedWin||50;var grade=tc.grade||'C';
var gradeC=grade.startsWith('A')?'#059669':grade.startsWith('B')?'#1a56db':'#d97706';
var winC=win>=65?'#059669':win>=50?'#d97706':'#dc2626';
var trd=d.trend||{};var vix=Number((d.options||{}).vix||0);
var eng=d.engines||{};var regime=(eng.regime||{}).trend||'';
var h='';

h+='<td colspan="9" style="padding:4px 6px">';
h+='<div style="border-radius:14px;overflow:hidden;border:1px solid var(--border);background:var(--surface);box-shadow:0 2px 8px rgba(0,0,0,.04);transition:all .3s" onmouseover="this.style.boxShadow=\'0 8px 28px rgba(0,0,0,.08)\';this.style.transform=\'translateY(-2px)\'" onmouseout="this.style.boxShadow=\'0 2px 8px rgba(0,0,0,.04)\';this.style.transform=\'none\'">';

// ── Layer 1: HEADLINE — instant scan ──
h+='<div style="display:flex;align-items:stretch">';

// Left: Identity + Market State
h+='<div style="padding:12px 14px;flex:1;border-right:1px solid var(--border);display:flex;align-items:center;gap:10px">';
// Grade with glow
h+='<div style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,'+gradeC+','+gradeC+'aa);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 3px 12px '+gradeC+'25"><span style="font-size:16px;font-weight:900;color:#fff">'+grade+'</span></div>';
h+='<div>';
h+='<div style="font-size:14px;font-weight:800;color:var(--text);letter-spacing:-.3px">'+d.symbol+'</div>';
h+='<div style="font-size:9px;color:var(--text3);font-family:var(--mono)">'+S+(d.spot||0).toLocaleString("en-IN")+'</div>';
// Regime headline
var _rLabel=regime==='BULLISH'?'BULL':regime==='BEARISH'?'BEAR':'RANGE';
var _rC=regime==='BULLISH'?'#059669':regime==='BEARISH'?'#dc2626':'#d97706';
h+='<div style="display:flex;align-items:center;gap:4px;margin-top:3px">';
h+='<span style="font-size:7px;font-weight:800;padding:1px 6px;border-radius:4px;background:'+_rC+'10;color:'+_rC+'">'+_rLabel+'</span>';
if(vix>0){h+='<span style="font-size:7px;font-weight:700;padding:1px 5px;border-radius:4px;background:'+(vix<20?'#1a56db08':'#dc262608')+';color:'+(vix<20?'#1a56db':'#dc2626')+';font-family:var(--mono)">VIX '+vix.toFixed(0)+'</span>';}
if(d.isExpiry){h+='<span style="font-size:7px;font-weight:800;padding:1px 5px;border-radius:4px;background:#dc262610;color:#dc2626">EXP</span>';}
h+='</div>';
h+='</div></div>';

// Center: Layer 2 — Action (What to do)
h+='<div style="padding:12px 14px;flex:1.3;border-right:1px solid var(--border);display:flex;align-items:center;justify-content:center">';
if(tr&&!isWait){
h+='<div style="display:flex;align-items:center;gap:8px;padding:6px 12px;border-radius:10px;background:'+dC+'06;border:1px solid '+dC+'15;width:100%">';
h+='<div style="width:28px;height:28px;border-radius:7px;background:'+dC+';display:flex;align-items:center;justify-content:center;flex-shrink:0"><span style="font-size:14px;color:#fff;font-weight:900">'+(dir==='BULLISH'?'▲':'▼')+'</span></div>';
h+='<div style="flex:1;min-width:0"><div style="font-size:11px;font-weight:800;color:'+dC+'">BUY '+tr.strike+' '+tr.type+'</div>';
h+='<div style="font-size:8px;color:var(--text3);font-family:var(--mono)">'+S+(tr.premEntry||0).toFixed(0)+' → SL '+S+(tr.premSL||0).toFixed(0)+' → T '+S+(tr.premT2||0).toFixed(0)+'</div></div>';
h+='<div style="text-align:center;padding:3px 6px;border-radius:5px;background:var(--bg2)"><div style="font-size:6px;color:var(--text3)">R:R</div><div style="font-size:12px;font-weight:900;color:#7c3aed;font-family:var(--mono)">'+(tr.rrRatio||'—')+'</div></div>';
h+='</div>';
}else{
h+='<div style="display:flex;align-items:center;gap:6px"><span style="font-size:16px">⏸️</span><span style="font-size:10px;font-weight:700;color:#d97706">WAIT</span></div>';
}
h+='</div>';

// Right: Layer 3 — Confidence (How sure)
h+='<div style="padding:12px 14px;width:90px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px">';
h+='<div style="position:relative;width:46px;height:46px">';
h+='<svg width="46" height="46" viewBox="0 0 46 46"><circle cx="23" cy="23" r="18" fill="none" stroke="var(--bg2)" stroke-width="3.5"/>';
var _cL2=2*3.14159*18;var _cO2=_cL2*(1-win/100);
h+='<circle cx="23" cy="23" r="18" fill="none" stroke="'+winC+'" stroke-width="3.5" stroke-linecap="round" stroke-dasharray="'+_cL2+'" stroke-dashoffset="'+_cO2+'" transform="rotate(-90 23 23)"/>';
h+='<text x="23" y="21" text-anchor="middle" font-size="12" font-weight="900" fill="'+winC+'" font-family="var(--mono)">'+win+'</text>';
h+='<text x="23" y="30" text-anchor="middle" font-size="5" fill="var(--text3)">%WIN</text></svg></div>';
h+='<span style="font-size:7px;font-weight:700;padding:1px 6px;border-radius:3px;background:'+winC+'10;color:'+winC+'">'+sig+'</span>';
h+='</div>';

h+='</div></div></td>';

// Expandable detail
h+='</tr><tr class="tt-detail-'+d.symbol+'" style="display:none"><td colspan="9" style="padding:0 6px 6px">';
h+='<div style="border-radius:0 0 14px 14px;border:1px solid var(--border);border-top:none;background:var(--bg2);padding:12px 16px">';
// Plans
var _pls=d.plans||[];
if(_pls.length>0){
h+='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">';
_pls.forEach(function(pl){
h+='<div style="flex:1;min-width:140px;padding:8px 10px;border-radius:10px;background:var(--surface);border:1px solid '+pl.color+'15;box-shadow:0 1px 4px rgba(0,0,0,.03)">';
h+='<div style="display:flex;align-items:center;gap:4px;margin-bottom:3px"><span style="width:18px;height:18px;border-radius:50%;background:linear-gradient(135deg,'+pl.color+','+pl.color+'cc);display:inline-flex;align-items:center;justify-content:center;font-size:8px;font-weight:900;color:#fff">'+pl.plan+'</span><span style="font-size:8px;font-weight:800;color:'+pl.color+'">'+pl.name+'</span></div>';
h+='<div style="font-size:8px;font-weight:700;color:var(--text)">'+pl.strategy+'</div>';
h+='<div style="font-size:7px;color:var(--text3);margin-top:1px">Loss: '+S+(typeof pl.maxLoss==="number"?pl.maxLoss.toLocaleString("en-IN"):pl.maxLoss)+'</div>';
h+='</div>';
});
h+='</div>';
}
...
if(tc.notes){h+='<div style="flex:1;min-width:160px;padding:8px;border-radius:8px;background:var(--surface);border:1px solid var(--border);font-size:8px">';
h+='<div style="font-weight:700;color:var(--text);margin-bottom:3px">📊 Score breakdown:</div>';
tc.notes.slice(0,3).forEach(function(n){var nc=n.indexOf('+')>=0?'#059669':n.indexOf('-')>=0?'#dc2626':'var(--text3)';h+='<div style="color:'+nc+';line-height:1.5">'+n+'</div>';});
h+='</div>';}
if(sc&&sc.direction&&sc.direction!=='NEUTRAL'){var scC2=sc.direction==='BULLISH'?'#059669':'#dc2626';
h+='<div style="flex:1;min-width:160px;padding:8px;border-radius:8px;background:'+scC2+'04;border:1px solid '+scC2+'12;font-size:8px">';
h+='<div style="font-weight:800;color:'+scC2+'">⚡ Scalp: '+sc.action+'</div>';
h+='<div style="color:var(--text3);margin-top:2px">'+sc.confidence+'% · Entry:'+S+sc.entry+' SL:'+S+sc.sl+'</div></div>';}
h+='</div>';
if(tr&&!isWait){h+='<div style="margin-top:6px;font-size:8px;color:var(--text3);padding:6px 8px;border-radius:6px;background:var(--surface);border:1px solid var(--border)">🎓 '+(dir==='BULLISH'?d.symbol+' expected UP → buy '+(isUS?'CALL':'CE'):d.symbol+' expected DOWN → buy '+(isUS?'PUT':'PE'))+'. Pay '+S+(tr.premEntry||0).toFixed(0)+', exit at '+S+(tr.premSL||0).toFixed(0)+' if wrong, profit at '+S+(tr.premT2||0).toFixed(0)+'.</div>';}
h+='</div></td>';
return h;
}
...
window._globalRegion=reg;
document.getElementById('scanRegIN').style.background=reg==='IN'?'#1a56db':'var(--bg2)';
document.getElementById('scanRegIN').style.color=reg==='IN'?'#fff':'var(--text3)';
document.getElementById('scanRegUS').style.background=reg==='US'?'#1a56db':'var(--bg2)';
...
var bg=s.chg>=2?'#047857':s.chg>=0.5?'#10b981':s.chg>=-0.5?'#6b7280':s.chg>=-2?'#ef4444':'#991b1b';
...
h+='</div>';
h+='<div style="display:flex;gap:6px;margin-top:8px;align-items:center;font-size:8px;color:var(--text3)"><span style="width:12px;height:12px;background:#991b1b;border-radius:2px"></span>&lt;-2%<span style="width:12px;height:12px;background:#ef4444;border-radius:2px;margin-left:4px"></span>-2 to -0.5%<span style="width:12px;height:12px;background:#6b7280;border-radius:2px;margin-left:4px"></span>Flat<span style="width:12px;height:12px;background:#10b981;border-radius:2px;margin-left:4px"></span>+0.5 to +2%<span style="width:12px;height:12px;background:#047857;border-radius:2px;margin-left:4px"></span>&gt;+2%</div>';
el.innerHTML=h;
...
var sigC=sig.includes('BUY')?'#10b981':sig.includes('OVER')?'#ef4444':sig==='MOMENTUM'?'var(--blue)':sig.includes('GAINER')?'#047857':'var(--text3)';
...
document.querySelectorAll('.algo-btn').forEach(function(b){b.classList.remove('active')});
if(btn)btn.classList.add('active');
// Highlight trade card in top trades grid — VERY visible
document.querySelectorAll('[id^="tc_"]').forEach(function(c){c.style.border='1px solid var(--border)';c.style.boxShadow='none';c.style.background='var(--surface)';c.style.transform='none'});
var tc=document.getElementById('tc_'+sym);
if(tc){tc.style.border='2px solid #1a56db';tc.style.boxShadow='0 0 24px rgba(26,86,219,.2),inset 0 0 0 1px rgba(26,86,219,.1)';tc.style.background='rgba(26,86,219,.04)';tc.style.transform='scale(1.02)';tc.scrollIntoView({behavior:'smooth',block:'nearest'})}
// Store active symbol
...
window._siRegion=reg;
document.getElementById('siRegIN').style.background=reg==='IN'?'#1a56db':'var(--bg2)';
document.getElementById('siRegIN').style.color=reg==='IN'?'#fff':'var(--text3)';
document.getElementById('siRegUS').style.background=reg==='US'?'#1a56db':'var(--bg2)';
...
if(!sym)return;
if(_siLoading){return}  // Prevent rapid clicks
_siLoading=true;
setTimeout(function(){_siLoading=false},3000);  // 3s cooldown between requests
...
// Left: Decision
h+='<div><div style="font-size:10px;color:var(--text3);font-weight:700;letter-spacing:1px;margin-bottom:4px">'+d.companyName+' · '+d.sector+'</div>';
h+='<div style="font-size:36px;font-weight:900;color:'+d.decColor+';font-family:Sora,sans-serif;letter-spacing:-1px">'+d.decision+'</div>';
h+='<div style="font-size:14px;font-weight:700;color:var(--text);margin-top:4px">'+d.symbol+' · '+d.csym+d.price.toLocaleString("en-IN")+'</div></div>';
// Right: Score circle
h+='<div style="text-align:center"><svg width="80" height="80" viewBox="0 0 80 80"><circle cx="40" cy="40" r="34" fill="none" stroke="var(--bg2)" stroke-width="5"/>';
var _cl=2*3.14159*34;var _co=_cl*(1-d.confidence/100);
h+='<circle cx="40" cy="40" r="34" fill="none" stroke="'+d.decColor+'" stroke-width="5" stroke-linecap="round" stroke-dasharray="'+_cl+'" stroke-dashoffset="'+_co+'" transform="rotate(-90 40 40)"/>';
h+='<text x="40" y="36" text-anchor="middle" font-size="20" font-weight="900" fill="'+d.decColor+'" font-family="var(--mono)">'+d.confidence+'</text>';
h+='<text x="40" y="50" text-anchor="middle" font-size="8" fill="var(--text3)">CONFIDENCE</text></svg></div>';
h+='</div>';
// Trade plan strip
if(d.tradePlan){var tp=d.tradePlan;
h+='<div style="display:flex;gap:0;margin-top:12px;border-radius:10px;overflow:hidden;font-size:10px;text-align:center">';
h+='<div style="flex:1;padding:8px;background:#05966910"><div style="font-size:7px;color:#059669;font-weight:700">ENTRY ZONE</div><div style="font-weight:800;color:#059669;font-family:var(--mono)">'+d.csym+tp.entry[0]+' – '+d.csym+tp.entry[1]+'</div></div>';
h+='<div style="flex:1;padding:8px;background:#dc262610"><div style="font-size:7px;color:#dc2626;font-weight:700">STOP LOSS</div><div style="font-weight:800;color:#dc2626;font-family:var(--mono)">'+d.csym+tp.sl+'</div></div>';
h+='<div style="flex:1;padding:8px;background:#05966910"><div style="font-size:7px;color:#059669;font-weight:700">TARGET 1</div><div style="font-weight:800;color:#059669;font-family:var(--mono)">'+d.csym+tp.t1+'</div></div>';
h+='<div style="flex:1;padding:8px;background:#04785710"><div style="font-size:7px;color:#047857;font-weight:700">TARGET 2</div><div style="font-weight:800;color:#047857;font-family:var(--mono)">'+d.csym+tp.t2+'</div></div>';
h+='<div style="flex:0.6;padding:8px;background:#7c3aed10"><div style="font-size:7px;color:#7c3aed;font-weight:700">R:R</div><div style="font-weight:800;color:#7c3aed;font-family:var(--mono)">1:'+tp.rr+'</div></div>';
h+='</div>';}
h+='</div></div>';

// ── 3-SCORE GAUGE ROW ──
var sections=[d.fundamental,d.valuation,d.priceAction];
var secNames=['Fundamentals','Valuation','Price Action'];
var secVerdicts=[d.fundamental.verdict,d.valuation.verdict,d.priceAction.trend];
var secColors=sections.map(function(s){return s.score>=65?'#059669':s.score>=40?'#d97706':'#dc2626'});
h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px">';
sections.forEach(function(sec,i){
h+='<div style="padding:14px;border-radius:12px;background:var(--surface);border:1px solid var(--border);text-align:center;box-shadow:var(--shadow-sm)">';
h+='<div style="font-size:9px;color:var(--text3);font-weight:700;margin-bottom:6px">'+secNames[i]+'</div>';
// Mini gauge
h+='<div style="position:relative;width:50px;height:50px;margin:0 auto 6px"><svg width="50" height="50" viewBox="0 0 50 50"><circle cx="25" cy="25" r="20" fill="none" stroke="var(--bg2)" stroke-width="4"/>';
var _mcl=2*3.14159*20;var _mco=_mcl*(1-sec.score/100);
h+='<circle cx="25" cy="25" r="20" fill="none" stroke="'+secColors[i]+'" stroke-width="4" stroke-linecap="round" stroke-dasharray="'+_mcl+'" stroke-dashoffset="'+_mco+'" transform="rotate(-90 25 25)"/>';
h+='<text x="25" y="28" text-anchor="middle" font-size="14" font-weight="900" fill="'+secColors[i]+'" font-family="var(--mono)">'+sec.score+'</text></svg></div>';
h+='<div style="font-size:10px;font-weight:800;color:'+secColors[i]+'">'+secVerdicts[i]+'</div>';
h+='</div>';
});
h+='</div>';

// ── DETAIL SECTIONS (Progressive Disclosure) ──
function siSection(title,emoji,content){
return '<details style="margin-bottom:8px;border:1px solid var(--border);border-radius:10px;overflow:hidden"><summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;font-weight:800;color:var(--text)">'+emoji+' '+title+'</span><span style="font-size:8px;color:var(--text3)">tap ▾</span></summary><div style="padding:12px 14px">'+content+'</div></details>';
}
...
// 3. Price Action
var pac='<div style="font-size:10px;color:var(--text);margin-bottom:6px"><strong>Trend:</strong> <span style="color:'+(d.priceAction.trend==='BULLISH'?'#059669':d.priceAction.trend==='BEARISH'?'#dc2626':'#d97706')+'">'+d.priceAction.trend+'</span> — '+d.priceAction.trendDetail+'</div>';
pac+='<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;font-size:9px">';
pac+='<div style="padding:6px;border-radius:6px;background:#05966906;border-left:3px solid #059669"><strong>Floor (Support)</strong><br>'+d.csym+d.priceAction.support.toLocaleString("en-IN")+'</div>';
pac+='<div style="padding:6px;border-radius:6px;background:#dc262606;border-left:3px solid #dc2626"><strong>Ceiling (Resistance)</strong><br>'+d.csym+d.priceAction.resistance.toLocaleString("en-IN")+'</div>';
pac+='<div style="padding:6px;border-radius:6px;background:var(--bg2)"><strong>Volume</strong>: '+d.priceAction.volConfirm+'</div>';
pac+='<div style="padding:6px;border-radius:6px;background:var(--bg2)"><strong>Breakout above</strong>: '+d.csym+d.priceAction.breakout.toLocaleString("en-IN")+'</div>';
pac+='</div>';
h+=siSection('Price Action — What is the chart saying?','📈',pac);
...
var etC=et.type==='BREAKOUT'?'#059669':et.type==='PULLBACK'?'#1a56db':'#d97706';
h+=siSection('Entry Timing — When to pull the trigger?','⏱️','<div style="padding:8px;border-radius:8px;background:'+etC+'08;border:1px solid '+etC+'20"><div style="font-size:11px;font-weight:800;color:'+etC+'">'+et.signal+'</div><div style="font-size:9px;color:var(--text2);margin-top:2px">'+et.condition+'</div></div>');

// 10. Position Sizing
var szC=d.sizing.level==='AGGRESSIVE'?'#059669':d.sizing.level==='MODERATE'?'#d97706':'#6b7280';
h+=siSection('Position Sizing — How much to bet?','📐','<div style="font-size:12px;font-weight:800;color:'+szC+'">'+d.sizing.level+'</div><div style="font-size:9px;color:var(--text2)">'+d.sizing.detail+'</div>');
...
h+=siSection('Risk Factors — What could go wrong?','🛑',rfc);

// 13. Invalidation
h+=siSection('Invalidation — When to abandon this idea','❌','<div style="font-size:10px;color:var(--text);line-height:1.6;padding:8px;border-radius:8px;background:#dc262606;border:1px solid #dc262612">'+d.invalidation+'</div>');

// 7. Simple Explanation (always visible at bottom)
h+='<div style="padding:14px;border-radius:12px;background:linear-gradient(135deg,rgba(8,145,178,.06),rgba(8,145,178,.02));border:1px solid rgba(8,145,178,.15);margin-top:8px">';
h+='<div style="font-size:10px;font-weight:800;color:#0891b2;margin-bottom:6px">🎓 In Simple Words</div>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6;margin-bottom:6px"><strong>Why:</strong> '+d.simple.why+'</div>';
h+='<div style="font-size:10px;color:#dc2626;line-height:1.6"><strong>Risk:</strong> '+d.simple.risk+'</div>';
h+='</div>';
...
window._valRegion=reg;
var inBtn=document.getElementById('valRegIN');var usBtn=document.getElementById('valRegUS');
if(inBtn){inBtn.style.background=reg==='IN'?'#1a56db':'var(--bg2)';inBtn.style.color=reg==='IN'?'#fff':'var(--text3)';}
if(usBtn){usBtn.style.background=reg==='US'?'#1a56db':'var(--bg2)';usBtn.style.color=reg==='US'?'#fff':'var(--text3)';}
...
var h='';
// Header
h+='<div style="padding:14px;border-radius:10px;background:linear-gradient(135deg,rgba(124,58,237,.04),rgba(59,130,246,.03));border:1px solid var(--border);margin-bottom:14px">';
h+='<div style="font-size:18px;font-weight:900;color:var(--text);margin-bottom:4px">'+sym+' — '+S+p.toLocaleString("en-IN")+'</div>';
// Valuation zone badge
var vzC=vl.zone&&(vl.zone.includes('EXPENSIVE'))?'#ef4444':vl.zone&&vl.zone.includes('UNDER')?'#10b981':'#f59e0b';
if(vl.zone){h+='<span style="font-size:10px;padding:3px 12px;border-radius:4px;background:'+vzC+'15;color:'+vzC+';font-weight:800">'+vl.zone+'</span>';}
h+='</div>';
// PE Metrics — visual gauge cards
h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:14px">';
if(vl.pe>0){
var peC=vl.pe>(vl.histPE?vl.histPE.avg:22)*1.2?'#ef4444':vl.pe<(vl.histPE?vl.histPE.avg:22)*0.8?'#10b981':'var(--text)';
h+='<div style="padding:14px;border-radius:10px;background:var(--surface);border:2px solid '+peC+'30;text-align:center">';
h+='<div style="font-size:8px;color:var(--text3);font-weight:700;letter-spacing:1px">TRAILING PE</div>';
h+='<div style="font-size:28px;font-weight:900;font-family:var(--mono);color:'+peC+'">'+vl.pe.toFixed(1)+'x</div>';
// Visual gauge bar
if(vl.histPE){
var pePct=Math.min(100,Math.max(0,((vl.pe-vl.histPE.low)/(vl.histPE.high-vl.histPE.low))*100));
h+='<div style="height:6px;border-radius:3px;background:linear-gradient(90deg,#10b981,#f59e0b,#ef4444);margin:6px 0;position:relative"><div style="position:absolute;top:-3px;left:'+pePct+'%;width:4px;height:12px;background:#fff;border:2px solid '+peC+';border-radius:2px;transform:translateX(-50%)"></div></div>';
h+='<div style="display:flex;justify-content:space-between;font-size:7px;color:var(--text3)"><span>Low '+vl.histPE.low+'x</span><span>Avg '+vl.histPE.avg+'x</span><span>High '+vl.histPE.high+'x</span></div>';
}
h+='<div style="font-size:8px;color:var(--text3);margin-top:4px">Like buying a shop: you pay ₹'+vl.pe.toFixed(0)+' for every ₹1 profit</div>';
h+='</div>';
}
if(vl.fwdPE>0){
var fC=vl.fwdPE<vl.pe?'#10b981':'#ef4444';
h+='<div style="padding:14px;border-radius:10px;background:var(--surface);border:2px solid '+fC+'30;text-align:center">';
h+='<div style="font-size:8px;color:var(--text3);font-weight:700;letter-spacing:1px">FORWARD PE</div>';
h+='<div style="font-size:28px;font-weight:900;font-family:var(--mono);color:'+fC+'">'+vl.fwdPE+'x</div>';
h+='<div style="font-size:9px;color:'+fC+';font-weight:700;margin-top:4px">'+(vl.fwdPE<vl.pe?'&#128994; Earnings GROWING — getting cheaper':'&#128308; Earnings SHRINKING — getting expensive')+'</div>';
h+='</div>';
}
if(vl.peg>0){
var pgC=vl.peg<1?'#10b981':vl.peg<2?'#f59e0b':'#ef4444';
h+='<div style="padding:14px;border-radius:10px;background:var(--surface);border:2px solid '+pgC+'30;text-align:center">';
h+='<div style="font-size:8px;color:var(--text3);font-weight:700;letter-spacing:1px">PEG RATIO</div>';
h+='<div style="font-size:28px;font-weight:900;font-family:var(--mono);color:'+pgC+'">'+vl.peg+'</div>';
h+='<div style="font-size:9px;color:'+pgC+';font-weight:700;margin-top:4px">'+(vl.peg<1?'Undervalued for growth rate':vl.peg<2?'Fair price for growth':'Overvalued')+'</div>';
h+='<div style="font-size:8px;color:var(--text3);margin-top:2px">PEG &lt; 1 = stock is cheap for its growth speed</div>';
h+='</div>';
}
if(vl.fairValue>0){
var fvC=p<vl.fairValue?'#10b981':'#ef4444';
h+='<div style="padding:14px;border-radius:10px;background:var(--surface);border:2px solid '+fvC+'30;text-align:center">';
h+='<div style="font-size:8px;color:var(--text3);font-weight:700;letter-spacing:1px">FAIR VALUE</div>';
h+='<div style="font-size:28px;font-weight:900;font-family:var(--mono);color:'+fvC+'">'+S+vl.fairValue.toLocaleString("en-IN")+'</div>';
h+='<div style="font-size:9px;color:'+fvC+';font-weight:700;margin-top:4px">'+(p<vl.fairValue?'Trading BELOW fair value ✅ ('+Math.round((1-p/vl.fairValue)*100)+'% discount)':'Trading ABOVE fair value ⚠️ ('+Math.round((p/vl.fairValue-1)*100)+'% premium)')+'</div>';
h+='</div>';
}
h+='</div>';
// Explanation
if(vl.explanation){h+='<div style="padding:10px 14px;border-radius:8px;background:var(--bg2);border:1px solid var(--border);margin-bottom:14px;font-size:10px;color:var(--text2);line-height:1.6">'+vl.explanation+(vl.fwdAnalysis?'<br><span style="color:var(--cyan)">'+vl.fwdAnalysis+'</span>':'')+'</div>';}
// Crash scenarios
if(vl.crashScenarios&&vl.crashScenarios.length>0){
h+='<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:800;color:#ef4444;margin-bottom:8px">&#128680; What If Markets Crash?</div>';
h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">';
vl.crashScenarios.forEach(function(cs){
var csC=cs.isBuyZone?'#10b981':'#ef4444';
h+='<div style="padding:12px;border-radius:10px;background:'+csC+'06;border:2px solid '+csC+'25;text-align:center">';
h+='<div style="font-size:24px;font-weight:900;color:'+csC+'">-'+cs.correction+'%</div>';
h+='<div style="font-size:16px;font-weight:800;font-family:var(--mono);color:var(--text)">'+S+cs.price.toLocaleString("en-IN")+'</div>';
if(cs.pe>0){h+='<div style="font-size:9px;color:var(--text3)">PE drops to '+cs.pe+'x</div>';}
h+='<div style="font-size:8px;margin-top:4px;padding:3px 8px;border-radius:4px;display:inline-block;background:'+(cs.isBuyZone?'#10b98115':'#ef444415')+';color:'+(cs.isBuyZone?'#10b981':'#ef4444')+';font-weight:800">'+(cs.isBuyZone?'BUY ZONE ✅':'STILL EXPENSIVE')+'</div>';
h+='<div style="font-size:8px;color:var(--text3);margin-top:4px">Need +'+cs.recovery+'% to recover</div>';
h+='<div style="font-size:8px;color:var(--text2);margin-top:4px;line-height:1.4">'+cs.scenario+'</div>';
h+='</div>';
});
h+='</div></div>';
}
// Geo risks
if(vl.geoRisks&&vl.geoRisks.length>0){
h+='<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:800;color:#f59e0b;margin-bottom:8px">&#127758; Geopolitical & Macro Risks</div>';
h+='<div style="display:grid;gap:6px">';
vl.geoRisks.forEach(function(gr){
var gC=gr.impact==='HIGH'?'#ef4444':gr.impact==='BULLISH'?'#10b981':'#f59e0b';
h+='<div style="padding:8px 12px;border-radius:8px;border-left:3px solid '+gC+';background:'+gC+'05">';
h+='<div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:10px;font-weight:700;color:var(--text)">'+gr.risk+'</span><span style="font-size:7px;padding:2px 8px;border-radius:3px;background:'+gC+'15;color:'+gC+';font-weight:700">'+gr.impact+'</span></div>';
h+='<div style="font-size:9px;color:var(--text3);margin-top:2px">Sectors: '+gr.sectors+'</div>';
h+='<div style="font-size:9px;color:var(--cyan);margin-top:1px">&#8594; '+gr.action+'</div>';
h+='</div>';
});
h+='</div></div>';
}
// Safe havens
if(vl.safeHavens&&vl.safeHavens.length>0){
h+='<div style="margin-bottom:14px"><div style="font-size:12px;font-weight:800;color:#10b981;margin-bottom:8px">&#128737; Where to Invest — Worst Case Playbook</div>';
vl.safeHavens.forEach(function(sh){
h+='<div style="padding:8px 12px;border-radius:8px;border-left:3px solid #10b981;background:rgba(16,185,129,.03);margin-bottom:6px">';
h+='<div style="font-size:10px;font-weight:800;color:var(--text)">'+sh.category+'</div>';
h+='<div style="font-size:12px;font-weight:800;color:var(--cyan);font-family:var(--mono)">'+sh.picks+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">'+sh.reason+'</div>';
h+='</div>';
});
h+='<div style="padding:8px;border-radius:6px;background:var(--bg2);font-size:9px;color:var(--text3);line-height:1.5">&#128161; The BEST strategy in any crash: <strong>SIP</strong> (invest fixed amount monthly). When markets fall, your SIP buys MORE units at LOWER prices. When they recover, those cheap units multiply. This is how ordinary people build extraordinary wealth.</div>';
h+='</div>';
}
el.innerHTML=h;
}).catch(function(e){el.innerHTML='<div style="color:var(--red);padding:12px">Error: '+e.message+'</div>'});
}
...
// ═══ AI ASSISTANT ═══
window._aiRegion='IN';
function switchAIRegion(reg){
window._aiRegion=reg;
var inBtn=document.getElementById('aiRegIN');var usBtn=document.getElementById('aiRegUS');
var inStocks=document.getElementById('aiStocksIN');var usStocks=document.getElementById('aiStocksUS');
if(inBtn){inBtn.style.background=reg==='IN'?'#1a56db':'var(--bg2)';inBtn.style.color=reg==='IN'?'#fff':'var(--text3)';}
if(usBtn){usBtn.style.background=reg==='US'?'#1a56db':'var(--bg2)';usBtn.style.color=reg==='US'?'#fff':'var(--text3)';}
if(inStocks)inStocks.style.display=reg==='IN'?'flex':'none';
if(usStocks)usStocks.style.display=reg==='US'?'flex':'none';
var sym=document.getElementById('aiSymbol');
if(sym)sym.value=reg==='US'?'SPY':'NIFTY';
}
function setAISym(s){
var el=document.getElementById('aiSymbol');if(el)el.value=s;
}
...
if(!q)return;
var reg=window._aiRegion||'IN';
var el=document.getElementById('aiChatArea');
if(!el)return;
// Clear screen for fresh answer
el.innerHTML='';
// Show question
el.innerHTML+='<div style="text-align:right;margin-bottom:10px"><span style="display:inline-block;padding:8px 14px;border-radius:12px 12px 2px 12px;background:var(--blue);color:#fff;font-size:11px;font-weight:600;max-width:70%">'+sym+': '+q+'</span></div>';
// Loading
...
var sigBadge='';
if(data.signal&&data.signal!=='N/A'){
var sCl=data.signal.includes('BUY')?'#10b981':data.signal==='AVOID'?'#ef4444':'#f59e0b';
sigBadge='<div style="margin-bottom:6px"><span style="font-size:8px;padding:3px 8px;border-radius:4px;background:'+sCl+'15;color:'+sCl+';font-weight:800">'+data.signal+'</span> <span style="font-size:8px;padding:3px 8px;border-radius:4px;background:'+(data.direction==='BULLISH'?'#10b98115':'#ef444415')+';color:'+(data.direction==='BULLISH'?'#10b981':'#ef4444')+';font-weight:800">'+data.direction+'</span></div>';
}
el.innerHTML+='<div style="margin-bottom:10px"><div style="display:inline-block;padding:12px 16px;border-radius:2px 14px 14px 14px;background:var(--bg2);color:var(--text);font-size:11px;max-width:90%;line-height:1.7;border:1px solid var(--border)">'+sigBadge+'<div style="white-space:pre-wrap">'+answer+'</div></div></div>';
el.scrollTop=el.scrollHeight;
}).catch(function(e){
var ld=document.getElementById('aiLoading');if(ld)ld.remove();
el.innerHTML+='<div style="margin-bottom:10px"><span style="display:inline-block;padding:10px 14px;border-radius:2px 12px 12px 12px;background:#ef444410;color:#ef4444;font-size:10px;border:1px solid #ef444430">Error: '+e.message+'</span></div>';
});
}
...
function tip(text){return '<div style="font-size:8px;color:var(--text3);margin-top:3px;padding:4px 8px;border-radius:4px;background:var(--bg2);line-height:1.5;border-left:2px solid var(--cyan)">\U0001f4a1 '+text+'</div>';}
...
var sC=sig==='STRONG BUY'?'#047857':sig==='BUY'?'#10b981':sig==='LEAN BUY'?'#3b82f6':sig==='AVOID'?'#ef4444':'#f59e0b';
var dC=dir==='BULLISH'?'#047857':dir==='BEARISH'?'#ef4444':'#f59e0b';
var isWait=sig==='HOLD / WAIT'||sig==='AVOID';
var tr=d.trade;var t=d.technicals||{};var orb=d.orb||{};var nse=d.options||{};var bs=d.black_scholes||{};
var h='';

// ═══ VISUAL STORY — 3-Layer Decision Card ═══
var _tc2=d.tradeConfidence||{};var _sc2=d.scalp||{};
var _win=_tc2.estimatedWin||50;var _grade=_tc2.grade||'C';
var _canScalp=_sc2&&_sc2.direction&&_sc2.direction!=='NEUTRAL'&&_sc2.confidence>=40;
var _lightC=_win>=65?'#059669':_win>=50?'#d97706':'#dc2626';
var _trd=d.trend||{};var _vix=Number((d.options||{}).vix||0);
var _tPct=_trd.pct||0;var _tC=_tPct>=60?'#047857':_tPct>=25?'#059669':_tPct>=-25?'#d97706':'#dc2626';
var _vxC=_vix<14?'#059669':_vix<20?'#1a56db':_vix<25?'#d97706':'#dc2626';
var _eng=d.engines||{};

// ── LAYER 1: HEADLINE — What's happening? (< 1 second read) ──
var _headlineText=(_trd.label||'ANALYZING').toUpperCase();
var _headlineSub='';
if(_tPct>=40)_headlineSub='Strong trend — momentum favors continuation';
else if(_tPct>=15)_headlineSub='Mild trend — proceed with caution';
else if(_tPct>=-15)_headlineSub='Range-bound — sell premium or wait for breakout';
else if(_tPct>=-40)_headlineSub='Mild bearish drift — shorts may work';
else _headlineSub='Strong downtrend — bears in control';

h+='<div style="margin-bottom:14px;border-radius:16px;overflow:hidden;border:1px solid var(--border);box-shadow:var(--shadow-lg)">';

// Headline bar
h+='<div style="padding:16px 20px;background:linear-gradient(135deg,'+_tC+'10,'+_tC+'04)">';
h+='<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">';
// Left: Market State
h+='<div>';
h+='<div style="font-size:28px;font-weight:900;color:'+_tC+';font-family:Sora,sans-serif;letter-spacing:-1px;line-height:1">'+_headlineText+'</div>';
h+='<div style="font-size:11px;color:var(--text2);margin-top:4px">'+_headlineSub+'</div>';
h+='<div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">';
(_trd.details||[]).slice(0,5).forEach(function(td){var _ic=td.dir==="BULL"?"#059669":td.dir==="BEAR"?"#dc2626":"#6b7280";h+='<span style="font-size:7px;font-weight:700;padding:2px 8px;border-radius:100px;background:'+_ic+'10;color:'+_ic+';border:1px solid '+_ic+'20">'+td.ind+'</span>';});
h+='</div></div>';
// Right: Price + VIX
h+='<div style="text-align:right">';
h+='<div style="font-size:24px;font-weight:900;color:var(--text);font-family:var(--mono);letter-spacing:-1px">'+S+p.toLocaleString("en-IN")+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">'+d.symbol+' · '+d.timestamp+'</div>';
if(_vix>0){
var _vxLabel=_vix<14?'Market calm':_vix<20?'Normal conditions':_vix<25?'Getting nervous':'High fear — expect big swings';
h+='<div style="margin-top:6px;display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:8px;background:'+_vxC+'08;border:1px solid '+_vxC+'15">';
h+='<span style="font-size:7px;font-weight:700;color:var(--text3)">FEAR</span>';
h+='<span style="font-size:14px;font-weight:900;font-family:var(--mono);color:'+_vxC+'">'+_vix.toFixed(1)+'</span>';
h+='<span style="font-size:7px;color:'+_vxC+';font-weight:600">'+_vxLabel+'</span>';
h+='</div>';
}
h+='</div></div>';
// Trend gauge
h+='<div style="margin-top:10px;height:6px;border-radius:3px;background:linear-gradient(90deg,#dc2626 0%,#d97706 30%,#059669 60%,#047857 100%);position:relative">';
var _np0=Math.max(3,Math.min(97,50+(_tPct/2)));
h+='<div style="position:absolute;top:-5px;left:'+_np0+'%;width:6px;height:16px;background:#fff;border:2px solid '+_tC+';border-radius:3px;transform:translateX(-50%);box-shadow:0 2px 6px rgba(0,0,0,.2)"></div></div>';
h+='<div style="display:flex;justify-content:space-between;margin-top:2px;font-size:6px;color:var(--text3)"><span>Sellers winning</span><span>Balanced</span><span>Buyers winning</span></div>';
h+='</div>';

// ── LAYER 2: CONTEXT — What should I do? ──
h+='<div style="padding:14px 20px;background:var(--surface);border-top:1px solid var(--border);border-bottom:1px solid var(--border)">';
if(tr&&!isWait){
h+='<div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap">';
// Direction block
h+='<div style="width:52px;height:52px;border-radius:14px;background:linear-gradient(135deg,'+dC+','+dC+'cc);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 4px 14px '+dC+'30"><span style="font-size:26px;color:#fff">'+(dir==='BULLISH'?'▲':'▼')+'</span></div>';
// Strategy text
h+='<div style="flex:1;min-width:140px">';
h+='<div style="font-size:20px;font-weight:900;color:'+dC+';font-family:Sora,sans-serif;letter-spacing:-.5px">BUY '+tr.strike+' '+(tr.type||'')+'</div>';
h+='<div style="font-size:10px;color:var(--text3);margin-top:2px">'+(dir==='BULLISH'?'Price expected to rise — profit when stock goes up':'Price expected to fall — profit when stock drops')+'</div>';
if(tr.expiry){h+='<div style="font-size:9px;color:var(--text3);margin-top:1px">Expires <strong style="color:var(--text)">'+tr.expiry+'</strong></div>';}
h+='</div>';
// Premium flow — arrow shapes
h+='<div style="display:flex;align-items:center;gap:0;flex-shrink:0">';
h+='<div style="padding:8px 10px;border-radius:8px 0 0 8px;background:#047857;text-align:center;min-width:56px"><div style="font-size:6px;color:rgba(255,255,255,.8);font-weight:700">ENTER</div><div style="font-size:15px;font-weight:900;color:#fff;font-family:var(--mono)">'+S+(tr.premEntry||0).toFixed(0)+'</div></div>';
h+='<div style="width:0;height:0;border-top:20px solid transparent;border-bottom:20px solid transparent;border-left:8px solid #047857"></div>';
h+='<div style="padding:8px 10px;background:#dc2626;text-align:center;min-width:50px"><div style="font-size:6px;color:rgba(255,255,255,.8);font-weight:700">EXIT IF WRONG</div><div style="font-size:15px;font-weight:900;color:#fff;font-family:var(--mono)">'+S+(tr.premSL||0).toFixed(0)+'</div></div>';
h+='<div style="width:0;height:0;border-top:20px solid transparent;border-bottom:20px solid transparent;border-left:8px solid #dc2626"></div>';
h+='<div style="padding:8px 10px;border-radius:0 8px 8px 0;background:#059669;text-align:center;min-width:56px"><div style="font-size:6px;color:rgba(255,255,255,.8);font-weight:700">BOOK PROFIT</div><div style="font-size:15px;font-weight:900;color:#fff;font-family:var(--mono)">'+S+(tr.premT2||0).toFixed(0)+'</div></div>';
h+='</div>';
h+='</div>';
// Copy + R:R
window._lastTradeForCopy=tr;
h+='<div style="display:flex;gap:8px;align-items:center;margin-top:8px">';
h+='<button onclick="copyBrokerOrder(window._lastTradeForCopy)" style="padding:5px 14px;border-radius:8px;background:var(--bg2);border:1px solid var(--border);color:var(--text);font-size:9px;font-weight:700;cursor:pointer;font-family:Plus Jakarta Sans,sans-serif;transition:all .2s">📋 Copy to Broker</button>';
h+='<span style="font-size:9px;color:var(--text3)">Risk:Reward <strong style="color:#7c3aed;font-size:13px;font-family:var(--mono)">'+(tr.rrRatio||'—')+'</strong></span>';
h+='<span style="font-size:8px;color:var(--text3)">For every ₹1 risk, earn ₹'+(tr.rrRatio?tr.rrRatio.split(':')[1]:'?')+'</span>';
h+='</div>';
}else{
h+='<div style="display:flex;align-items:center;gap:12px;padding:14px;border-radius:12px;background:#d9770608;border:1px solid #d9770620">';
h+='<span style="font-size:36px">⏸️</span>';
h+='<div><div style="font-size:18px;font-weight:900;color:#d97706;font-family:Sora,sans-serif">SIT THIS ONE OUT</div>';
h+='<div style="font-size:10px;color:var(--text2)">Signals are mixed — like 50% rain forecast. No edge to trade.</div>';
if(_canScalp){h+='<div style="font-size:10px;color:var(--cyan);margin-top:4px;font-weight:700">⚡ But a quick scalp exists: '+_sc2.direction+' ('+_sc2.confidence+'% confidence)</div>';}
h+='</div></div>';
}
h+='</div>';

// ── LAYER 3: CONFIDENCE — How confident? (Progressive disclosure) ──
h+='<div style="padding:14px 20px;background:var(--bg2)">';

// Confidence bar — the main visual
...
h+='<svg width="64" height="64" viewBox="0 0 64 64"><circle cx="32" cy="32" r="26" fill="none" stroke="var(--border)" stroke-width="5"/>';
var _circ=2*3.14159*26;var _offset=_circ*(1-_win/100);
h+='<circle cx="32" cy="32" r="26" fill="none" stroke="'+_lightC+'" stroke-width="5" stroke-linecap="round" stroke-dasharray="'+_circ+'" stroke-dashoffset="'+_offset+'" transform="rotate(-90 32 32)"/>';
h+='<text x="32" y="30" text-anchor="middle" font-size="16" font-weight="900" fill="'+_lightC+'" font-family="var(--mono)">'+_win+'</text>';
h+='<text x="32" y="41" text-anchor="middle" font-size="7" fill="var(--text3)" font-weight="600">%WIN</text></svg></div>';

// Engine scores
h+='<div style="flex:1">';
var _factors=[
{name:'Trend',weight:30,score:(_eng.regime||{}).score||50,tip:'Market direction strength'},
{name:'Smart Money',weight:25,score:(_eng.oi||{}).score||50,tip:'Where institutions are positioned'},
{name:'Volatility',weight:20,score:(_eng.volatility||{}).score||50,tip:'Are options cheap or expensive?'},
{name:'Momentum',weight:25,score:(_eng.momentum||{}).score||50,tip:'Entry timing signals'}
];
// Use engine final score
if(_eng&&_eng.finalScore)_win=Math.round(_eng.finalScore);
_factors.forEach(function(f){
var fC=f.score>=65?'#059669':f.score>=45?'#d97706':'#dc2626';
h+='<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">';
h+='<span style="font-size:8px;font-weight:600;color:var(--text3);width:72px">'+f.name+' <span style="opacity:.6">('+f.weight+'%)</span></span>';
h+='<div style="flex:1;height:7px;border-radius:4px;background:var(--bg3);overflow:hidden"><div style="height:100%;width:'+f.score+'%;border-radius:4px;background:linear-gradient(90deg,'+fC+','+fC+'bb);transition:width .6s ease"></div></div>';
h+='<span style="font-size:9px;font-weight:800;font-family:var(--mono);color:'+fC+';width:24px;text-align:right">'+f.score+'</span>';
h+='</div>';
});
h+='</div>';

// Grade + Verdict
var _gradeC=_grade.startsWith('A')?'#059669':_grade.startsWith('B')?'#1a56db':'#d97706';
h+='<div style="text-align:center;min-width:60px">';
h+='<div style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,'+_gradeC+','+_gradeC+'cc);display:inline-flex;align-items:center;justify-content:center;box-shadow:0 3px 10px '+_gradeC+'30"><span style="font-size:17px;font-weight:900;color:#fff">'+_grade+'</span></div>';
h+='<div style="font-size:7px;color:var(--text3);font-weight:700;margin-top:3px">GRADE</div></div>';
h+='</div>';

// Coach verdict — layman one-liner
var _verdict=_win>=70?'All lights green — this is a strong setup. Trade with full confidence.':_win>=55?'Mostly green lights — decent setup. Use smaller position size.':_win>=40?'Mixed signals — wait for clarity or try the scalp alternative.':'Mostly red lights — step aside. Protecting capital IS a strategy.';
h+='<div style="padding:8px 12px;border-radius:8px;background:var(--surface);border:1px solid '+_lightC+'20;font-size:9px;color:var(--text2);line-height:1.6">';
h+='<span style="font-weight:800;color:'+_lightC+'">'+(_win>=70?'✅':_win>=55?'🟡':'🔴')+' Coach says:</span> '+_verdict;
if(_tc2.notes&&_tc2.notes.length>0){
h+=' <span style="color:var(--text3)">';
_tc2.notes.slice(0,3).forEach(function(n){var nc=n.indexOf('+')>=0?'#059669':n.indexOf('-')>=0?'#dc2626':'var(--text3)';h+='<span style="color:'+nc+'">'+n+'</span> · ';});
h+='</span>';
}
h+='</div>';

// Expected move range bar
var em=_eng.expectedMove;
if(em&&em.points>0){
h+='<div style="margin-top:8px;padding:8px 12px;border-radius:8px;background:var(--surface);border:1px solid var(--border)">';
h+='<div style="font-size:8px;font-weight:700;color:var(--text3);margin-bottom:4px">📐 Market expects price to stay between these walls ('+em.dte+' days)</div>';
h+='<div style="display:flex;align-items:center;gap:6px;font-size:10px">';
h+='<span style="font-weight:800;color:#dc2626;font-family:var(--mono)">'+S+em.low.toLocaleString("en-IN")+'</span>';
h+='<div style="flex:1;height:14px;border-radius:7px;background:linear-gradient(90deg,#dc262615 0%,#d9770615 25%,#05966915 50%,#d9770615 75%,#dc262615 100%);position:relative;border:1px solid var(--border)">';
var emRange=em.high-em.low;var emPct=emRange>0?((p-em.low)/emRange)*100:50;
emPct=Math.max(3,Math.min(97,emPct));
h+='<div style="position:absolute;top:50%;left:'+emPct+'%;width:3px;height:20px;background:var(--text);border-radius:2px;transform:translate(-50%,-50%);box-shadow:0 0 6px rgba(0,0,0,.2)"></div>';
h+='<div style="position:absolute;top:50%;left:'+emPct+'%;transform:translate(-50%,-150%);font-size:7px;font-weight:700;color:var(--text);white-space:nowrap">'+S+p.toLocaleString("en-IN")+'</div>';
h+='</div>';
h+='<span style="font-weight:800;color:#059669;font-family:var(--mono)">'+S+em.high.toLocaleString("en-IN")+'</span>';
h+='</div></div>';
}
...
if(gex.length>0){
h+='<details style="margin-top:6px"><summary style="cursor:pointer;font-size:8px;font-weight:700;color:var(--text3);padding:4px 0">⚡ Smart Money Zones ('+gex.length+' levels)</summary>';
h+='<div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">';
gex.forEach(function(g){var gC=g.type==='SUPPORT'?'#059669':'#dc2626';h+='<span style="font-size:7px;padding:3px 8px;border-radius:6px;background:'+gC+'08;border:1px solid '+gC+'20;color:'+gC+';font-weight:700">'+(g.type==='SUPPORT'?'🟢 Buyers at ':'🔴 Sellers at ')+S+g.strike.toLocaleString("en-IN")+'</span>';});
h+='</div></details>';
}

h+='</div></div>';

// ═══ FUNDAMENTALS — Company Health Report ═══
var _fa=d.fundamentals||null;
if(_fa&&(_fa.roe>0||_fa.revGrowth)){
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:12px;overflow:hidden">';
h+='<summary style="padding:12px 16px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center">';
h+='<span style="font-size:11px;font-weight:800;color:var(--text)">📊 COMPANY HEALTH — Is This a Good Business?</span>';
h+='<span style="font-size:9px;padding:3px 10px;border-radius:4px;background:'+_fa.color+'12;color:'+_fa.color+';font-weight:800">'+_fa.grade+'</span></summary>';
h+='<div style="padding:14px 16px">';
// Score bar
h+='<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:8px;margin-bottom:3px"><span style="color:var(--text3)">Fundamental Score</span><span style="font-weight:800;color:'+_fa.color+'">'+_fa.score+'/100</span></div>';
h+='<div style="height:8px;border-radius:4px;background:var(--bg3);overflow:hidden"><div style="height:100%;width:'+_fa.score+'%;border-radius:4px;background:'+_fa.color+'"></div></div></div>';
// Metrics grid
h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:6px;margin-bottom:10px">';
var _faMetrics=[
{l:'Revenue Growth',v:_fa.revGrowth+'%',c:_fa.revGrowth>10?'#059669':_fa.revGrowth<0?'#dc2626':'#d97706'},
{l:'Earnings Growth',v:_fa.earnGrowth+'%',c:_fa.earnGrowth>10?'#059669':_fa.earnGrowth<0?'#dc2626':'#d97706'},
{l:'Profit Margin',v:_fa.profitMargin+'%',c:_fa.profitMargin>15?'#059669':_fa.profitMargin<5?'#dc2626':'#d97706'},
{l:'Debt/Equity',v:_fa.deRatio+'%',c:_fa.deRatio<80?'#059669':_fa.deRatio>150?'#dc2626':'#d97706'},
{l:'ROE',v:_fa.roe+'%',c:_fa.roe>15?'#059669':_fa.roe<5?'#dc2626':'#d97706'},
{l:'ROA',v:_fa.roa+'%',c:_fa.roa>5?'#059669':'#d97706'},
];
_faMetrics.forEach(function(m){
h+='<div style="padding:6px 8px;border-radius:8px;background:'+m.c+'06;border:1px solid '+m.c+'15;text-align:center">';
h+='<div style="font-size:7px;color:var(--text3);font-weight:700">'+m.l+'</div>';
h+='<div style="font-size:14px;font-weight:900;font-family:var(--mono);color:'+m.c+'">'+m.v+'</div></div>';
});
h+='</div>';
// Red flags & strengths
if(_fa.strengths&&_fa.strengths.length>0){h+='<div style="margin-bottom:6px">';_fa.strengths.forEach(function(s){h+='<div style="font-size:8px;color:#059669;padding:2px 0">✅ '+s+'</div>';});h+='</div>';}
if(_fa.redFlags&&_fa.redFlags.length>0){h+='<div style="margin-bottom:6px">';_fa.redFlags.forEach(function(r){h+='<div style="font-size:8px;color:#dc2626;padding:2px 0">⚠️ '+r+'</div>';});h+='</div>';}
// Coach summary
h+='<div style="font-size:9px;color:var(--text2);padding:8px 10px;border-radius:6px;background:var(--bg2);line-height:1.5">💡 '+_fa.coachSummary+'</div>';
h+='</div></details>';
}
...
h+='<div><div style="font-size:12px;font-weight:800;color:#059669">NEAR BREAKOUT — Price approaching ceiling at '+S+_bo.resistance.toLocaleString("en-IN")+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">If price crosses above with volume → expect strong upward move. Watch for a close above this level.</div></div></div>';
}
if(_bo.nearBreakdown){
h+='<div style="margin-bottom:10px;padding:10px 14px;border-radius:10px;background:rgba(220,38,38,.06);border:2px solid rgba(220,38,38,.2);display:flex;align-items:center;gap:10px">';
h+='<span style="font-size:24px">⚠️</span>';
h+='<div><div style="font-size:12px;font-weight:800;color:#dc2626">NEAR BREAKDOWN — Price approaching floor at '+S+_bo.support.toLocaleString("en-IN")+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">If price falls below with volume → expect sharp drop. Consider puts or exiting longs.</div></div></div>';
}
}

// ═══ 3-PLAN DECISION — Visual Trade Picker ═══
var _plans=d.plans||[];
if(_plans.length>0){
h+='<div style="margin-bottom:14px;border-radius:16px;overflow:hidden;border:1px solid var(--border);box-shadow:var(--shadow)">';
h+='<div style="padding:14px 18px;background:linear-gradient(135deg,rgba(26,86,219,.06),rgba(124,58,237,.04));border-bottom:1px solid var(--border)">';
h+='<div style="font-size:13px;font-weight:800;color:var(--text)">🎯 3 WAYS TO TRADE '+d.symbol+'</div>';
h+='<div style="font-size:9px;color:var(--text3);margin-top:2px">Each plan is a complete recipe — follow step by step like a cooking instruction</div></div>';

_plans.forEach(function(pl,idx){
var isOpen=idx===0;
h+='<div style="border-bottom:1px solid var(--border)">';
// Plan header — clickable accordion
h+='<div onclick="var c=this.nextElementSibling;c.style.display=c.style.display===\'none\'?\'block\':\'none\';this.querySelector(\'.plarr\').textContent=c.style.display===\'none\'?\'▶\':\'▼\'" style="padding:12px 18px;cursor:pointer;display:flex;align-items:center;gap:10px;transition:background .2s" onmouseover="this.style.background=\''+pl.color+'06\'" onmouseout="this.style.background=\'transparent\'">';
h+='<div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,'+pl.color+','+pl.color+'bb);display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 3px 10px '+pl.color+'30"><span style="font-size:15px;font-weight:900;color:#fff">'+pl.plan+'</span></div>';
h+='<div style="flex:1"><div style="font-size:12px;font-weight:800;color:'+pl.color+'">'+pl.name+'</div>';
h+='<div style="font-size:9px;color:var(--text3)">'+pl.tagline+'</div></div>';
// Risk badge + arrow
var rskC2=pl.risk==='HIGH'?'#dc2626':pl.risk==='MEDIUM'?'#d97706':'#059669';
h+='<span style="font-size:7px;padding:3px 8px;border-radius:4px;background:'+rskC2+'12;color:'+rskC2+';font-weight:700">'+pl.risk+' RISK</span>';
h+='<span class="plarr" style="font-size:10px;color:var(--text3)">'+(isOpen?'▼':'▶')+'</span>';
h+='</div>';

// Plan body — expandable
h+='<div style="display:'+(isOpen?'block':'none')+';padding:0 18px 16px">';

// Strategy name in highlight box
h+='<div style="padding:10px 14px;border-radius:10px;background:'+pl.color+'08;border:1px solid '+pl.color+'20;margin-bottom:12px">';
h+='<div style="font-size:11px;font-weight:800;color:'+pl.color+'">'+pl.strategy+'</div>';
h+='</div>';

// ★ STEP-BY-STEP TRADE INSTRUCTIONS — like a recipe
h+='<div style="margin-bottom:12px">';
h+='<div style="font-size:9px;font-weight:800;color:var(--text);margin-bottom:8px;letter-spacing:.5px">📋 STEP-BY-STEP — How to Place This Trade:</div>';

if(pl.type==='DIRECTIONAL'){
// Plan A: Simple buy
h+='<div style="display:flex;flex-direction:column;gap:6px">';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #1a56db">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#1a56db;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">1</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Open your broker app</strong> (Zerodha/Angel/IBKR) and search for <strong style="color:'+pl.color+'">'+pl.strategy.replace("BUY ","")+'</strong></div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">2</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Click BUY</strong> and enter quantity. Premium should be around <strong style="color:#059669">'+S+pl.entry+'</strong>. This is your cost per share/unit.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #dc2626">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#dc2626;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">3</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Set Stop Loss at '+S+pl.sl+'</strong> — if the premium drops to this price, your broker will auto-sell to limit your loss. <em style="color:var(--text3)">This is your safety net.</em></div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">4</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>When premium reaches '+S+pl.t1+'</strong> → sell HALF your position (book partial profit). Move stop loss to entry price — <strong style="color:#059669">now it is a FREE trade!</strong></div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:#05966908;border-left:3px solid #047857">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#047857;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">5</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Let remaining ride to '+S+pl.t2+'</strong> — this is your big profit target. Trail your stop loss up as price moves in your favor.</div></div>';
h+='</div>';

}else if(pl.type==='SPREAD'){
// Plan B: 2-leg spread
var parts=pl.strategy.split('/');
var leg1=parts[0]||pl.strategy;
var leg2=parts[1]||'';
h+='<div style="display:flex;flex-direction:column;gap:6px">';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #1a56db">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#1a56db;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">1</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>LEG 1 — BUY:</strong> In your broker, search for the <strong style="color:#059669">LOWER strike</strong> option and click <strong>BUY</strong>. You PAY premium for this leg.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #dc2626">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#dc2626;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">2</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>LEG 2 — SELL:</strong> Search for the <strong style="color:#dc2626">HIGHER strike</strong> option and click <strong>SELL</strong>. You RECEIVE premium for this leg. <em style="color:var(--text3)">This reduces your cost.</em></div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #7c3aed">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#7c3aed;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">3</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Your net cost = '+S+pl.entry+'</strong> (what you paid MINUS what you received). This is your MAXIMUM possible loss. No surprises.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:#05966908;border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">4</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Just wait until expiry.</strong> Max profit = '+pl.maxProfit+'. No stop loss needed — your max loss is already locked at '+S+pl.capital.toLocaleString("en-IN")+'.</div></div>';
h+='</div>';

}else{
// Plan C: Non-directional (Iron Condor or Straddle)
var isIC=pl.strategy.indexOf('IRON')>=0;
if(isIC){
h+='<div style="display:flex;flex-direction:column;gap:6px">';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #dc2626">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#dc2626;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">1</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>SELL a CE (Call)</strong> above current price — you COLLECT money for this. You are betting price WON\'T go above this level.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #dc2626">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#dc2626;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">2</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>SELL a PE (Put)</strong> below current price — you COLLECT more money. You are betting price WON\'T go below this level.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">3</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>BUY protection wings</strong> (1 CE higher + 1 PE lower) — cheap insurance in case of extreme moves. This makes it an IRON CONDOR.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:#7c3aed08;border-left:3px solid #7c3aed">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#7c3aed;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">4</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Collect your credit of '+S+Math.abs(pl.entry)+'</strong> and WAIT. If price stays in the range by expiry, you keep ALL the money.'+(pl.pop?' <strong style="color:#1a56db">~'+pl.pop+'% chance of winning.</strong>':'')+'</div></div>';
h+='</div>';
}else{
// Straddle
h+='<div style="display:flex;flex-direction:column;gap:6px">';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">1</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>BUY one CE (Call)</strong> at the strike nearest current price. You PAY premium.</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:var(--bg2);border-left:3px solid #059669">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#059669;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">2</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>BUY one PE (Put)</strong> at the SAME strike. You PAY more premium. Total cost = '+S+pl.entry+'</div></div>';
h+='<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;border-radius:8px;background:#7c3aed08;border-left:3px solid #7c3aed">';
h+='<span style="width:22px;height:22px;border-radius:50%;background:#7c3aed;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:900;color:#fff;flex-shrink:0">3</span>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.6"><strong>Now you WIN whichever way price moves</strong> — big up or big down. You just need it to move MORE than '+S+pl.entry+' from current price. Only lose if market is flat.</div></div>';
h+='</div>';
}
}
h+='</div>';

// Capital + Risk summary row
h+='<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:8px">';
h+='<div style="padding:8px;border-radius:8px;background:#dc262606;text-align:center;border:1px solid #dc262612"><div style="font-size:7px;color:#dc2626;font-weight:700">MAX YOU CAN LOSE</div><div style="font-size:14px;font-weight:900;color:#dc2626;font-family:var(--mono)">'+S+(typeof pl.maxLoss==="number"?pl.maxLoss.toLocaleString("en-IN"):pl.maxLoss)+'</div></div>';
h+='<div style="padding:8px;border-radius:8px;background:#05966906;text-align:center;border:1px solid #05966912"><div style="font-size:7px;color:#059669;font-weight:700">MAX YOU CAN MAKE</div><div style="font-size:14px;font-weight:900;color:#059669;font-family:var(--mono)">'+pl.maxProfit+'</div></div>';
h+='<div style="padding:8px;border-radius:8px;background:var(--bg2);text-align:center;border:1px solid var(--border)"><div style="font-size:7px;color:var(--text3);font-weight:700">CAPITAL NEEDED</div><div style="font-size:14px;font-weight:900;color:var(--text);font-family:var(--mono)">'+S+pl.capital.toLocaleString("en-IN")+'</div></div>';
h+='</div>';

// Analogy
h+='<div style="padding:8px 12px;border-radius:8px;background:rgba(26,86,219,.04);border:1px solid rgba(26,86,219,.1);font-size:9px;color:var(--text2);line-height:1.6">💡 '+pl.analogy+'</div>';
...
// Valuation moved to separate tab — show compact link
var vl=d.valuation||null;
if(vl&&vl.pe>0){
var vzC2=vl.zone&&vl.zone.includes('EXPENSIVE')?'#ef4444':vl.zone&&vl.zone.includes('UNDER')?'#10b981':'#f59e0b';
h+='<div style="margin-bottom:10px;padding:8px 14px;border-radius:8px;background:'+vzC2+'06;border:1px solid '+vzC2+'25;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">';
h+='<div><span style="font-size:10px;font-weight:800;color:var(--text)">Valuation: </span><span style="font-size:10px;padding:2px 8px;border-radius:4px;background:'+vzC2+'15;color:'+vzC2+';font-weight:800">'+vl.zone+'</span> <span style="font-size:10px;color:var(--text3)">PE: '+vl.pe.toFixed(1)+'x'+(vl.fwdPE?' | Fwd: '+vl.fwdPE+'x':'')+(vl.fairValue?' | Fair: '+S+vl.fairValue.toLocaleString("en-IN"):'')+'</span></div>';
h+='<button onclick="switchTabGroup(\'trading\');setTimeout(function(){switchTab(\'valreport\');loadValuation(\''+d.symbol+'\')},100)" style="padding:3px 10px;border-radius:4px;background:var(--blue);color:#fff;border:none;font-size:8px;font-weight:700;cursor:pointer">Full Report →</button>';
h+='</div>';
}

// ═══ SIDE-BY-SIDE: SCALP (Quick) vs INTRADAY (Swing) ═══
var sc=d.scalp||null;
if(tr||sc){
var hasScalp=sc&&sc.direction!=='NEUTRAL'&&sc.confidence>=40;
var hasTrade=tr&&!isWait;
var scC=hasScalp?(sc.direction==='BULLISH'?'#059669':'#dc2626'):'#6b7280';
var scS=isUS?'$':'&#8377;';

h+='<div style="margin-bottom:14px;border-radius:14px;overflow:hidden;border:1px solid var(--border);box-shadow:var(--shadow)">';
h+='<div style="padding:10px 18px;background:var(--bg2);border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">';
h+='<div style="font-size:12px;font-weight:800;color:var(--text)">⚡ TRADE COMPARISON — Pick Your Style</div>';
h+='<div style="font-size:8px;color:var(--text3)">Same stock, two approaches</div></div>';

h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:0">';

// ── LEFT: SCALP (Quick Trade) ──
h+='<div style="padding:14px;border-right:1px solid var(--border);'+(hasScalp?'':'opacity:.4')+'">';
h+='<div style="display:flex;align-items:center;gap:6px;margin-bottom:10px">';
h+='<div style="width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#f59e0b,#d97706);display:flex;align-items:center;justify-content:center"><span style="font-size:14px;color:#fff">⚡</span></div>';
h+='<div><div style="font-size:11px;font-weight:800;color:#d97706">SCALP — Quick Trade</div>';
h+='<div style="font-size:7px;color:var(--text3)">Hold: 5–30 minutes</div></div></div>';

if(hasScalp){
// Direction badge
h+='<div style="display:flex;gap:6px;align-items:center;margin-bottom:8px">';
h+='<span style="font-size:9px;font-weight:800;padding:3px 10px;border-radius:6px;background:'+scC+'12;color:'+scC+'">'+sc.direction+'</span>';
h+='<span style="font-size:14px;font-weight:900;font-family:var(--mono);color:'+scC+'">'+sc.confidence+'%</span>';
h+='</div>';

// Action
h+='<div style="font-size:10px;font-weight:800;color:'+scC+';margin-bottom:8px">'+sc.action+'</div>';

// Compact table
h+='<table style="width:100%;border-collapse:collapse;font-size:9px;margin-bottom:8px">';
h+='<tr><td style="padding:4px 6px;color:var(--text3);font-weight:700;width:55px">Entry</td><td style="padding:4px 6px;font-weight:800;color:var(--text);font-family:var(--mono)">'+scS+sc.entry.toLocaleString()+(sc.premEntry?' <span style="color:var(--cyan);font-size:8px">(₹'+sc.premEntry+')</span>':'')+'</td></tr>';
h+='<tr style="background:var(--bg2)"><td style="padding:4px 6px;color:#dc2626;font-weight:700">SL</td><td style="padding:4px 6px;font-weight:800;color:#dc2626;font-family:var(--mono)">'+scS+sc.sl.toLocaleString()+(sc.premSL?' <span style="color:var(--text3);font-size:8px">(₹'+sc.premSL+')</span>':'')+'</td></tr>';
h+='<tr><td style="padding:4px 6px;color:#059669;font-weight:700">Target 1</td><td style="padding:4px 6px;font-weight:800;color:#059669;font-family:var(--mono)">'+scS+sc.t1.toLocaleString()+(sc.premT1?' <span style="color:var(--text3);font-size:8px">(₹'+sc.premT1+')</span>':'')+'</td></tr>';
h+='<tr style="background:var(--bg2)"><td style="padding:4px 6px;color:#047857;font-weight:700">Target 2</td><td style="padding:4px 6px;font-weight:800;color:#047857;font-family:var(--mono)">'+scS+sc.t2.toLocaleString()+(sc.premT2?' <span style="color:var(--text3);font-size:8px">(₹'+sc.premT2+')</span>':'')+'</td></tr>';
h+='</table>';

// Context pills
h+='<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px">';
h+='<span style="font-size:7px;padding:2px 6px;border-radius:4px;background:var(--bg2);color:'+(sc.trend_5m==='BULLISH'?'#059669':'#dc2626')+';font-weight:700">5m: '+sc.trend_5m+'</span>';
h+='<span style="font-size:7px;padding:2px 6px;border-radius:4px;background:var(--bg2);color:'+(sc.trend_15m==='BULLISH'?'#059669':'#dc2626')+';font-weight:700">15m: '+sc.trend_15m+'</span>';
h+='<span style="font-size:7px;padding:2px 6px;border-radius:4px;background:var(--bg2);color:'+(sc.above_vwap?'#059669':'#dc2626')+';font-weight:700">VWAP: '+(sc.above_vwap?'Above ✓':'Below ✗')+'</span>';
if(sc.vol_surge>1.3)h+='<span style="font-size:7px;padding:2px 6px;border-radius:4px;background:rgba(6,182,212,.1);color:var(--cyan);font-weight:700">Vol '+sc.vol_surge+'x 🚀</span>';
h+='</div>';

h+='<div style="font-size:7px;color:var(--text3);line-height:1.4;padding:4px 6px;border-radius:4px;background:var(--bg2)">💡 Quick in-out (5-30 min). Enter on 5m candle close with volume. Book at T1, trail to T2.</div>';
}else{
h+='<div style="text-align:center;padding:20px;color:var(--text3);font-size:9px">⏸️ No scalp setup right now.<br>5m + 15m trends not aligned.</div>';
}
h+='</div>';

// ── RIGHT: INTRADAY (Full Trade) ──
h+='<div style="padding:14px;'+(hasTrade?'':'opacity:.4')+'">';
h+='<div style="display:flex;align-items:center;gap:6px;margin-bottom:10px">';
h+='<div style="width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,'+dC+','+dC+'cc);display:flex;align-items:center;justify-content:center"><span style="font-size:14px;color:#fff">'+(dir==='BULLISH'?'▲':'▼')+'</span></div>';
h+='<div><div style="font-size:11px;font-weight:800;color:'+dC+'">INTRADAY — Full Trade</div>';
h+='<div style="font-size:7px;color:var(--text3)">Hold: 1 hour – full day</div></div></div>';

if(hasTrade){
// Direction + signal
h+='<div style="display:flex;gap:6px;align-items:center;margin-bottom:8px">';
h+='<span style="font-size:9px;font-weight:800;padding:3px 10px;border-radius:6px;background:'+sC+'12;color:'+sC+'">'+sig+'</span>';
h+='<span style="font-size:14px;font-weight:900;font-family:var(--mono);color:'+dC+'">'+(d.tradeConfidence?d.tradeConfidence.estimatedWin:50)+'%</span>';
h+='</div>';

// Action
h+='<div style="font-size:10px;font-weight:800;color:'+dC+';margin-bottom:8px">'+tr.action+'</div>';

// Compact table
var pe2=tr.premEntry||0,ps2=tr.premSL||0,p12=tr.premT1||0,p22=tr.premT2||0;
h+='<table style="width:100%;border-collapse:collapse;font-size:9px;margin-bottom:8px">';
h+='<tr><td style="padding:4px 6px;color:var(--text3);font-weight:700;width:55px">Entry</td><td style="padding:4px 6px;font-weight:800;color:var(--text);font-family:var(--mono)">'+S+pe2.toFixed(1)+' <span style="color:var(--text3);font-size:8px">premium</span></td></tr>';
h+='<tr style="background:var(--bg2)"><td style="padding:4px 6px;color:#dc2626;font-weight:700">SL</td><td style="padding:4px 6px;font-weight:800;color:#dc2626;font-family:var(--mono)">'+S+ps2.toFixed(1)+' <span style="color:var(--text3);font-size:8px">(-'+S+(pe2-ps2).toFixed(0)+' loss)</span></td></tr>';
h+='<tr><td style="padding:4px 6px;color:#059669;font-weight:700">Target 1</td><td style="padding:4px 6px;font-weight:800;color:#059669;font-family:var(--mono)">'+S+p12.toFixed(1)+' <span style="color:var(--text3);font-size:8px">(+'+S+(p12-pe2).toFixed(0)+' profit)</span></td></tr>';
h+='<tr style="background:var(--bg2)"><td style="padding:4px 6px;color:#047857;font-weight:700">Target 2</td><td style="padding:4px 6px;font-weight:800;color:#047857;font-family:var(--mono)">'+S+p22.toFixed(1)+' <span style="color:var(--text3);font-size:8px">(+'+S+(p22-pe2).toFixed(0)+' profit)</span></td></tr>';
h+='<tr><td style="padding:4px 6px;color:var(--purple);font-weight:700">R:R</td><td style="padding:4px 6px;font-weight:900;color:var(--purple);font-family:var(--mono)">'+(tr.rrRatio||'—')+'</td></tr>';
h+='</table>';

// Contract info
h+='<div style="font-size:8px;color:var(--text3);margin-bottom:6px">'+tr.contractName+(tr.expiry?' · Exp: <strong style="color:var(--text)">'+tr.expiry+'</strong>':'')+'</div>';

window._lastTradeForCopy=tr;
h+='<div style="display:flex;gap:6px">';
h+='<button onclick="copyBrokerOrder(window._lastTradeForCopy)" style="padding:3px 10px;border-radius:6px;background:var(--bg2);border:1px solid var(--border);color:var(--text);font-size:8px;font-weight:700;cursor:pointer">📋 Copy Order</button>';
h+='</div>';
}else{
h+='<div style="text-align:center;padding:20px;color:var(--text3);font-size:9px">⏸️ Low confluence — no trade.<br>Only '+sup+'/'+tot+' factors support.</div>';
}
h+='</div>';
...
h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:0;border-top:1px solid var(--border);font-size:8px;text-align:center">';
h+='<div style="padding:6px;border-right:1px solid var(--border);background:rgba(217,119,6,.03)"><strong style="color:#d97706">SCALP</strong>: Fast in-out, smaller profit, tight SL. Like grabbing a taxi — quick ride to next corner.</div>';
h+='<div style="padding:6px;background:'+dC+'03"><strong style="color:'+dC+'">INTRADAY</strong>: Hold longer, bigger targets, wider SL. Like booking a ride across the city — takes time but goes further.</div>';
h+='</div>';
h+='</div>';
}
...
if(oi){
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden"><summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;font-weight:800;color:var(--cyan)">&#128202; OPTIONS — How Expensive Are They?</span><span style="font-size:8px;color:var(--text3)">tap to expand ▾</span></summary>';
...
if(strats.length>0){
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden"><summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;font-weight:800;color:var(--purple)">&#9853; MORE STRATEGIES — Other Ways to Play</span><span style="font-size:8px;color:var(--text3)">tap to expand ▾</span></summary>';
strats.forEach(function(st){
var stC=st.type==='DIRECTIONAL'?(dir==='BULLISH'?'#047857':'#ef4444'):'#7c3aed';
h+='<div style="padding:10px 14px;border-bottom:1px solid var(--border)">';
...
if(gd){
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden">';
h+='<summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center">';
h+='<span style="font-size:11px;font-weight:800;color:var(--amber)">🏥 OPTION HEALTH — Is Your Option Fit or Sick?</span>';
h+='<span style="font-size:8px;color:var(--text3)">tap ▾</span></summary>';
h+='<div style="padding:6px 14px;font-size:9px;color:var(--text2);line-height:1.5;background:var(--bg2);border-bottom:1px solid var(--border)">Think of Greeks as your option\'s health report: <strong>Delta</strong> = how fast it moves, <strong>Theta</strong> = how fast it\'s aging, <strong>Gamma</strong> = how unpredictable it is, <strong>Vega</strong> = how sensitive to market fear.</div>';
h+='<div style="font-size:9px;color:var(--text3);padding:4px 14px">Strike: '+S+gd.strike+' · DTE: '+gd.dte+'</div>';
// Greeks values
h+='<div style="padding:10px 14px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px;border-bottom:1px solid var(--border)">';
var greekItems=[
{label:'DELTA (Δ)',val:gd.ce.delta,color:gd.ce.delta>0?'#10b981':'#ef4444',sub:gd.ce.delta>0.6?'Moves fast with stock':'Moves slowly with stock',tip:'Speed — how much your option price moves when stock moves ₹1'},
{label:'GAMMA (Γ)',val:gd.ce.gamma,color:gd.ce.gamma>0.003?'#ef4444':'var(--text)',sub:gd.ce.gamma>0.003?'⚠️ Accelerating fast':'Steady speed',tip:'Acceleration — how fast Delta itself is changing. High near expiry.'},
{label:'THETA (Θ)',val:gd.ce.theta,color:'#ef4444',sub:'Losing '+S+Math.abs(gd.dailyDecay)+' per day',tip:'Time tax — your option loses this much value every day just by existing. Like ice cream melting.'},
{label:'VEGA (V)',val:gd.ce.vega,color:'#3b82f6',sub:gd.ce.vega>0.15?'Sensitive to fear changes':'Stable',tip:'Fear sensitivity — when market panic (VIX) rises, your option price jumps by this much.'},
];
...
h+='<div style="font-size:7px;color:'+gi.color+';font-weight:600">'+gi.sub+'</div>';
h+='<div style="font-size:6px;color:var(--text3);margin-top:2px;line-height:1.3">'+gi.tip+'</div></div>';
});
h+='</div>';
// Interpretations
...
if(vi){
var viC=vi.regime.includes('SELL')?'#dc2626':vi.regime.includes('BUY')?'#059669':vi.regime.includes('ELEV')?'#d97706':'var(--text)';
var viCoach=vi.regime.includes('SELL')?'Options are EXPENSIVE right now — like buying flight tickets during peak holiday. Sellers have the edge. Consider selling premium or spreads.':vi.regime.includes('BUY')?'Options are CHEAP — like getting a hotel room off-season. Great time to buy calls/puts. Premium buyers have the edge.':vi.regime.includes('ELEV')?'Options are slightly pricey. Not the worst time to buy, but consider spreads to reduce cost.':'Options are normally priced. Standard strategies work fine.';
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden"><summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;font-weight:800;color:#8b5cf6">🌡️ OPTIONS PRICE CHECK — Should You Buy or Sell Options?</span><span style="font-size:8px;color:var(--text3)">tap ▾</span></summary>';
// Coach summary at top
h+='<div style="padding:10px 14px;background:'+viC+'06;border-bottom:1px solid var(--border);font-size:10px;color:var(--text);line-height:1.6">'+viCoach+'</div>';
// Metrics
h+='<div style="padding:10px 14px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px">';
h+='<div style="text-align:center;padding:8px;border-radius:8px;background:var(--bg2)"><div style="font-size:7px;color:var(--text3);font-weight:700">OPTIONS PRICE</div><div style="font-size:20px;font-weight:900;font-family:var(--mono);color:'+viC+'">'+vi.iv+'%</div><div style="font-size:7px;color:var(--text3)">Currently '+(vi.iv>25?'expensive':'normal')+'</div></div>';
h+='<div style="text-align:center;padding:8px;border-radius:8px;background:var(--bg2)"><div style="font-size:7px;color:var(--text3);font-weight:700">NORMAL PRICE</div><div style="font-size:20px;font-weight:900;font-family:var(--mono);color:var(--text)">'+vi.hv20+'%</div><div style="font-size:7px;color:var(--text3)">Based on last 20 days</div></div>';
h+='<div style="text-align:center;padding:8px;border-radius:8px;background:var(--bg2)"><div style="font-size:7px;color:var(--text3);font-weight:700">MARKUP</div><div style="font-size:20px;font-weight:900;font-family:var(--mono);color:'+(vi.ivHvSpread>3?'#dc2626':vi.ivHvSpread<-3?'#059669':'var(--text)')+'">'+vi.ivHvSpread+'%</div><div style="font-size:7px;color:var(--text3)">'+(vi.ivHvSpread>3?'Overpriced ⚠️':vi.ivHvSpread<-3?'Underpriced ✅':'Fair')+'</div></div>';
h+='<div style="text-align:center;padding:8px;border-radius:8px;background:'+viC+'08;border:1px solid '+viC+'20"><div style="font-size:7px;color:'+viC+';font-weight:700">VERDICT</div><div style="font-size:11px;font-weight:800;color:'+viC+';margin-top:4px">'+vi.action+'</div></div>';
h+='</div>';
h+='<div style="padding:6px 14px 10px;font-size:8px;color:var(--text3);line-height:1.4;background:var(--bg2)">💡 Think of IV like a hotel room price. <strong>High IV</strong> = peak season (expensive rooms). <strong>Low IV</strong> = off-season (cheap rooms). The markup shows how much extra you are paying vs normal. Negative markup = bargain.</div>';
h+='</details>';
}
...
if(rm){
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden"><summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center"><span style="font-size:11px;font-weight:800;color:#ef4444">&#128737; HOW MUCH SHOULD I BET?</span><span style="font-size:8px;color:var(--text3)">tap to expand ▾</span></summary>';
// Exposure alerts
...
// ═══ EVIDENCE TABLE — COLLAPSED BY DEFAULT ═══
h+='<details style="margin-bottom:14px;border:1px solid var(--border);border-radius:10px;overflow:hidden">';
h+='<summary style="padding:10px 14px;background:var(--bg2);cursor:pointer;list-style:none;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">';
h+='<span style="font-size:11px;font-weight:800;color:var(--text)">&#128220; EVIDENCE — '+tot+' Expert Opinions</span>';
h+='<div style="display:flex;gap:6px;align-items:center"><span style="font-size:9px;font-weight:700;color:#047857">&#11044; '+sup+' for</span><span style="font-size:9px;font-weight:700;color:#ef4444">&#11044; '+opp+' against</span><span style="font-size:8px;color:var(--text3)">tap to see all ▾</span></div></summary>';
h+='<div style="font-size:8px;color:var(--text3);padding:6px 14px">Each row is like an expert opinion. Green = says BUY, Red = says SELL. More green + heavier weight = stronger signal.</div>';
h+='<div style="display:flex;gap:8px"><span style="font-size:10px;font-weight:700;color:#047857">&#11044; '+sup+' ('+wS.toFixed(1)+'pt)</span><span style="font-size:10px;font-weight:700;color:#ef4444">&#11044; '+opp+' ('+wO.toFixed(1)+'pt)</span><span style="font-size:10px;font-weight:700;color:#f59e0b">&#11044; '+neu+' ('+wN.toFixed(1)+'pt)</span></div></div>';
// Weighted progress bar
h+='<div style="height:8px;display:flex"><div style="height:100%;width:'+(wT>0?(wS/wT*100):0)+'%;background:#047857"></div><div style="height:100%;width:'+(wT>0?(wN/wT*100):0)+'%;background:#f59e0b"></div><div style="height:100%;width:'+(wT>0?(wO/wT*100):0)+'%;background:#ef4444"></div></div>';
// Table
...
var fc=f.status==='SUPPORTS'?'#047857':f.status==='OPPOSES'?'#ef4444':'#f59e0b';
...
// ═══ ② YOUR TRADE — Entry, Stop Loss, Targets SHOWN (with warning for AVOID) ═══
if(tr){
...
h+='</div></div>';
// Premium entry — big
h+='<div style="padding:14px;text-align:center;border-bottom:1px solid '+dC+'15"><div style="font-size:9px;color:var(--text3);font-weight:700;letter-spacing:1.5px">YOUR ENTRY PRICE (Option Premium)'+(tr.optionLTP?' <span style="background:#0eb24e;color:#fff;font-size:7px;padding:1px 5px;border-radius:3px;letter-spacing:0">LIVE LTP</span>':'')+'</div><div style="font-size:38px;font-weight:900;color:'+dC+';font-family:var(--mono)">'+S+pe.toFixed(1)+'</div>'+(tr.expiry?'<div style="font-size:10px;color:var(--text3);margin-top:2px">Expiry: <strong style="color:var(--text)">'+tr.expiry+'</strong> &middot; Contract: <strong style="color:var(--cyan)">'+tr.contractName+'</strong></div>':'')+'</div>';
// R:R bar
...
h+='</div></div>';
// Layman explanation of the trade ticket
h+='<div style="padding:6px 16px;font-size:8px;color:var(--text3);line-height:1.5;background:'+dC+'04">'+tip('The colored bar above is your trade plan. <strong>RED zone</strong> = your stop loss (exit here if wrong). <strong>GOLD</strong> = where you enter. <strong>GREEN zones</strong> = your profit targets. When price reaches T1, book half your profit and move stop loss to entry price — now it is a FREE trade!')+'</div>';
// 4-box
h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:0;border-bottom:1px solid '+dC+'15">';
h+='<div style="padding:10px;text-align:center;border-right:1px solid '+dC+'12;background:rgba(239,68,68,.03)"><div style="font-size:8px;font-weight:700;color:#ef4444">EXIT IF WRONG (SL)</div><div style="font-size:16px;font-weight:900;color:#ef4444;font-family:var(--mono)">&#8804; '+S+ps.toFixed(1)+'</div><div style="font-size:8px;color:var(--text3)">Loss '+S+(tr.premRisk||0)+'/qty</div></div>';
h+='<div style="padding:10px;text-align:center;border-right:1px solid '+dC+'12;background:rgba(10,124,66,.03)"><div style="font-size:8px;font-weight:700;color:#047857">BOOK PROFIT HERE</div><div style="font-size:16px;font-weight:900;color:#047857;font-family:var(--mono)">'+S+p2.toFixed(1)+'</div><div style="font-size:8px;color:var(--text3)">Profit '+S+(tr.premReward||0)+'/qty</div></div>';
h+='<div style="padding:10px;text-align:center;border-right:1px solid '+dC+'12"><div style="font-size:8px;font-weight:700;color:var(--text3)">MAX LOSS/LOT</div><div style="font-size:16px;font-weight:900;color:#ef4444;font-family:var(--mono)">'+S+(tr.riskPerLot||0).toLocaleString("en-IN")+'</div></div>';
...
h+='</div>';
h+=tip('R:R 1:2 means for every ₹1 you risk losing, you can make ₹2. Always aim for R:R above 1:1.5. Would you bet ₹100 to win ₹200? That is a good R:R.');
// 3-target strip
...
h+='<div style="padding:8px 12px;border-right:1px solid '+dC+'12"><strong style="color:#047857">T2 '+S+p2.toFixed(1)+'</strong><br><span style="color:var(--text3)">'+tr.t2Action+'</span></div>';
...
h+='<div><strong style="color:var(--blue)">&#9654; WHEN TO ENTER:</strong> <span style="color:var(--text2)">'+tr.entry+'</span></div>';
h+='<div><strong style="color:#ef4444">&#9632; WHEN TO EXIT:</strong> <span style="color:var(--text2)">'+tr.exit+'</span></div>';
h+=tip('ENTER = the exact moment to buy your option. WAIT for this condition. EXIT = when to sell if trade goes wrong. Place this BEFORE entering — not in your head, in the system. The market does not care about your feelings.');
h+='</div></div>';
}
...
if(t.pdh>0)h+='<div style="padding:8px;border-radius:6px;background:var(--bg2);text-align:center;border:1px solid var(--border)"><div style="font-size:7px;color:var(--text3);font-weight:700">PDH/PDL</div><div style="font-size:11px;font-weight:800;font-family:var(--mono);color:#047857">'+S+t.pdh.toLocaleString("en-IN")+'</div><div style="font-size:11px;font-weight:800;font-family:var(--mono);color:#ef4444">'+S+t.pdl.toLocaleString("en-IN")+'</div></div>';
h+='</div>';
// Layman explanation of technical metrics
h+='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;font-size:8px;line-height:1.4">';
if(orb.vwap>0)h+='<span style="padding:3px 6px;border-radius:4px;background:var(--bg2);color:var(--text3)"><strong>VWAP</strong> = Average price big players paid today. '+(p>orb.vwap?'Price ABOVE = buyers winning ✅':'Price BELOW = sellers winning ❌')+'</span>';
if(t.pivot>0)h+='<span style="padding:3px 6px;border-radius:4px;background:var(--bg2);color:var(--text3)"><strong>CPR</strong> = Todays balance zone. '+t.cpr_type+' = '+(t.cpr_type==='NARROW'?'big move expected today':'range-bound day likely')+'</span>';
if(nse.vix>0)h+='<span style="padding:3px 6px;border-radius:4px;background:var(--bg2);color:var(--text3)"><strong>VIX</strong> = Fear meter. '+(nse.vix<16?'Low fear = calm market 😌':nse.vix>25?'High fear = volatile! ⚠️':'Moderate anxiety')+'</span>';
if(nse.pcr>0)h+='<span style="padding:3px 6px;border-radius:4px;background:var(--bg2);color:var(--text3)"><strong>PCR</strong> = Put/Call ratio. '+(nse.pcr>1.2?'Lots of puts = contrarian bullish signal':'Lots of calls = crowd is bullish, be cautious')+'</span>';
if(nse.max_pain>0)h+='<span style="padding:3px 6px;border-radius:4px;background:var(--bg2);color:var(--text3)"><strong>Max Pain</strong> '+S+nse.max_pain.toLocaleString("en-IN")+' = Price where most options expire worthless. Market gravitates here on expiry.</span>';
h+='</div>';
h+='<div style="font-size:9px;font-weight:700;color:var(--text3);padding:4px 0;letter-spacing:0.5px">&#127919; KEY LEVELS — Where Big Players Are Positioned</div>';
// OI table
...
h+='<div style="padding:8px 12px;border-right:1px solid var(--border)"><div style="font-size:8px;font-weight:700;color:#ef4444;margin-bottom:4px">CEILING — Sellers Active Here ↑</div>'
h+='<div style="font-size:7px;color:var(--text3);margin-bottom:2px">\u2191 Price ceiling — hard to go above</div>';
...
h+='</div><div style="padding:8px 12px"><div style="font-size:8px;font-weight:700;color:#047857;margin-bottom:4px">FLOOR — Buyers Defending Here ↓</div>'
h+='<div style="font-size:7px;color:var(--text3);margin-bottom:2px">\u2193 Price floor — hard to fall below</div>';
...
// ═══ TREND — Which Way Is the Market Moving? ═══
h+='<div style="font-size:9px;font-weight:700;color:var(--text3);padding:0 0 4px;letter-spacing:0.5px">&#128200; TREND — Which Direction Is The Market Moving?</div>';
var tr_d=d.trend||{};var alerts=d.alerts||[];
if(tr_d.label){
var tPct=tr_d.pct||0;var tC2=tPct>=60?'#047857':tPct>=25?'#10b981':tPct>=-25?'#f59e0b':'#ef4444';
...
h+='<div style="flex:1;height:6px;background:linear-gradient(90deg,#ef4444,#f59e0b,#10b981,#047857);border-radius:3px;position:relative">';
...
(tr_d.details||[]).slice(0,5).forEach(function(td){var ic2=td.dir==='BULL'?'#047857':td.dir==='BEAR'?'#ef4444':'#94a3b8';h+='<span style="font-size:7px;font-weight:700;padding:1px 5px;border-radius:3px;background:'+ic2+'12;color:'+ic2+'">'+td.ind+'</span>';});
h+='</div>';
}
h+=tip('TREND = the market\'s general direction over days/weeks. Like checking if the tide is coming IN or going OUT before swimming. Always trade WITH the trend, not against it.');
if(alerts.length>0){
h+='<div style="margin-bottom:10px">';
alerts.slice(0,4).forEach(function(a){var ac2=a.type==='BULLISH'?'#047857':a.type==='BEARISH'?'#ef4444':a.type==='CRITICAL'?'#991b1b':'#f59e0b';h+='<div style="padding:5px 10px;border-radius:6px;background:'+ac2+'08;border-left:3px solid '+ac2+';margin-bottom:3px;font-size:9px"><strong style="color:'+ac2+'">'+a.type+':</strong> <span style="color:var(--text2)">'+a.msg+'</span></div>';});
h+='</div>';
}
...
h+='<div style="padding:8px"><div style="font-size:7px;color:#047857">TGT 2-3×</div><div style="font-weight:900;color:#047857;font-family:var(--mono)">'+blast.target.split(",")[0]+'</div></div>';
h+='</div>';
...
// Beginner summary
var bDir=dir==='BULLISH'?'expects price to GO UP \u2191':'expects price to GO DOWN \u2193';
var bOpt=dir==='BULLISH'?(isUS?'CALL':'CE'):(isUS?'PUT':'PE');
var bExplain=dir==='BULLISH'?'We buy a '+bOpt+' option — like placing a bet that '+d.symbol+' will rise. If it goes up, our option becomes more valuable. If wrong, we lose only the premium ('+S+(tr?tr.premEntry:0)+').':'We buy a '+bOpt+' option — like buying insurance against a fall. If '+d.symbol+' drops, our option gains value. Max loss = premium paid ('+S+(tr?tr.premEntry:0)+').';
h+='<div style="margin-top:10px;padding:12px;border-radius:8px;background:rgba(59,130,246,.04);border:1px solid rgba(59,130,246,.15)">';
h+='<div style="font-size:10px;font-weight:800;color:var(--blue);margin-bottom:6px">\ud83c\udf93 BEGINNER\'S SUMMARY</div>';
h+='<div style="font-size:10px;color:var(--text);line-height:1.7">';
h+='<strong>Direction:</strong> This stock '+bDir+'<br>';
h+='<strong>What we do:</strong> '+bExplain+'<br>';
if(tr){h+='<strong>Entry:</strong> Buy the option at '+S+(tr.premEntry||0)+' (this is your ticket cost)<br>';
h+='<strong>Stop Loss:</strong> Exit at '+S+(tr.premSL||0)+' if trade goes wrong (limit your loss)<br>';
h+='<strong>Target:</strong> Book profit at '+S+(tr.premT2||0)+' (your payday)<br>';
h+='<strong>Max Loss:</strong> '+S+(tr.premEntry||0)+' per unit (you can never lose more than this)<br>';}
h+='<strong>Key Rule:</strong> Never risk more than 2% of your total capital on one trade. If you have '+S+'5,00,000 — max loss per trade = '+S+'10,000.';
h+='</div></div>';
...
var stC=a.triggered?'#047857':'var(--text3)';
...
div.style.cssText='position:fixed;top:60px;right:20px;z-index:9999;padding:14px 20px;border-radius:10px;background:#1a56db;color:#fff;font-family:Sora,sans-serif;font-size:12px;font-weight:700;box-shadow:0 8px 32px rgba(0,0,0,.3);animation:fi .3s ease;max-width:320px';
...
var el=document.getElementById('backtestSection');
if(!el)return;
...
el.innerHTML='<div style="border-left:3px solid var(--purple);padding:0;margin-bottom:12px"><div style="padding:12px 16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:16px">&#128202;</span><span style="font-family:Sora,sans-serif;font-size:14px;font-weight:800;color:var(--text)">Backtest Engine</span><span style="font-size:8px;padding:2px 8px;border-radius:100px;background:var(--purple);color:#fff;font-weight:700">HISTORICAL PROOF</span></div><div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap"><select id="btSym" style="padding:5px 10px;border-radius:6px;border:1px solid var(--border);font-size:11px;font-family:DM Sans,sans-serif;background:var(--surface);color:var(--text)">'+opts+'</select><select id="btYears" style="padding:5px 10px;border-radius:6px;border:1px solid var(--border);font-size:11px;font-family:DM Sans,sans-serif;background:var(--surface);color:var(--text)"><option value="1">1 Year</option><option value="2">2 Years</option><option value="3" selected>3 Years</option><option value="5">5 Years</option></select><button id="btRunBtn" onclick="runBacktest()" style="padding:5px 16px;border-radius:6px;background:var(--purple);color:#fff;border:none;font-size:11px;font-weight:700;cursor:pointer;font-family:Sora,sans-serif">&#9889; Run Backtest</button></div></div></div><div id="btResult" style="padding:0 16px 16px"><div style="font-size:11px;color:var(--text3);text-align:center;padding:16px">Select instrument and click Run Backtest to validate the algo on historical data.</div></div>';
el.style.display='block';
}
...
function renderBacktestResults(el,d){
var s=d.stats;
var S='&#8377;';
var wrC=s.win_rate>=60?'#047857':s.win_rate>=50?'#f59e0b':'#ef4444';
var pfC=s.profit_factor>=1.5?'#047857':s.profit_factor>=1?'#f59e0b':'#ef4444';
var shC=s.sharpe_ratio>=1.5?'#047857':s.sharpe_ratio>=0.5?'#f59e0b':'#ef4444';
var netC=s.net_pnl>=0?'#047857':'#ef4444';

var h='';

// Header
h+='<div style="margin-bottom:14px"><div style="font-size:9px;color:var(--text3);font-weight:600;letter-spacing:1.5px">'+d.symbol+' &middot; '+d.period+' &middot; '+d.total_bars+' BARS &middot; CONFLUENCE ≥'+d.confluence_threshold+'</div>';
h+='<div style="font-size:14px;font-weight:800;color:var(--text);margin-top:4px">Backtest Results — '+s.total_trades+' Trades Over '+d.years+' Year'+(d.years>1?'s':'')+'</div></div>';

// Big stat cards
h+='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:8px;margin-bottom:14px">';
var cards=[
{l:'WIN RATE',v:s.win_rate+'%',c:wrC,sub:s.wins+'W / '+s.losses+'L'},
{l:'PROFIT FACTOR',v:s.profit_factor===Infinity?'∞':s.profit_factor.toFixed(2),c:pfC,sub:'Profit/Loss ratio'},
{l:'SHARPE RATIO',v:s.sharpe_ratio.toFixed(2),c:shC,sub:'Risk-adjusted return'},
{l:'NET P&L',v:(s.net_pnl>=0?'+':'')+S+Math.abs(s.net_pnl).toLocaleString('en-IN'),c:netC,sub:'Per lot'},
{l:'EXPECTANCY',v:(s.expectancy>=0?'+':'')+S+Math.abs(s.expectancy).toLocaleString('en-IN'),c:s.expectancy>=0?'#047857':'#ef4444',sub:'Per trade avg'},
{l:'MAX DRAWDOWN',v:S+s.max_drawdown.toLocaleString('en-IN'),c:'#ef4444',sub:'Worst peak-to-trough'},
{l:'AVG WIN',v:'+'+S+s.avg_win.toLocaleString('en-IN'),c:'#047857',sub:s.wins+' winning trades'},
{l:'AVG LOSS',v:S+Math.abs(s.avg_loss).toLocaleString('en-IN'),c:'#ef4444',sub:s.losses+' losing trades'},
];
cards.forEach(function(c){
h+='<div style="padding:10px;border-radius:8px;background:'+c.c+'08;border:1px solid '+c.c+'20;text-align:center">';
h+='<div style="font-size:7px;color:var(--text3);letter-spacing:1px;font-weight:700">'+c.l+'</div>';
h+='<div style="font-size:18px;font-weight:900;color:'+c.c+';font-family:var(--mono);margin:2px 0">'+c.v+'</div>';
h+='<div style="font-size:8px;color:var(--text3)">'+c.sub+'</div></div>';
});
h+='</div>';

// Win rate visual bar
h+='<div style="margin-bottom:14px"><div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:4px">Win/Loss Distribution</div>';
h+='<div style="display:flex;height:24px;border-radius:6px;overflow:hidden;font-size:9px;font-weight:700">';
var wPct=s.total_trades>0?(s.wins/s.total_trades*100):0;
var lPct=s.total_trades>0?(s.losses/s.total_trades*100):0;
var bPct=100-wPct-lPct;
if(wPct>0)h+='<div style="background:#047857;color:#fff;width:'+wPct+'%;display:flex;align-items:center;justify-content:center;min-width:30px">'+s.wins+'W</div>';
if(bPct>0&&s.breakevens>0)h+='<div style="background:#f59e0b;color:#fff;width:'+bPct+'%;display:flex;align-items:center;justify-content:center;min-width:20px">'+s.breakevens+'B</div>';
if(lPct>0)h+='<div style="background:#ef4444;color:#fff;width:'+lPct+'%;display:flex;align-items:center;justify-content:center;min-width:30px">'+s.losses+'L</div>';
h+='</div></div>';

// Equity curve
if(d.equity_curve&&d.equity_curve.length>10){
h+='<div style="margin-bottom:14px"><div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:6px">Equity Curve (cumulative P&L per lot)</div>';
h+='<div style="height:120px;background:var(--bg2);border-radius:8px;border:1px solid var(--border);padding:8px;position:relative;overflow:hidden">';
...
var x=(i/(eq.length-1))*100;
var y=100-((eq[i]-eqMin)/eqRange)*100;
pts.push(x.toFixed(1)+','+y.toFixed(1));
}
h+='<svg viewBox="0 0 100 100" preserveAspectRatio="none" style="width:100%;height:100%">';
// Zero line
var zeroY=100-((0-eqMin)/eqRange)*100;
h+='<line x1="0" y1="'+zeroY.toFixed(1)+'" x2="100" y2="'+zeroY.toFixed(1)+'" stroke="var(--text3)" stroke-width="0.3" stroke-dasharray="1,1"/>';
h+='<polyline points="'+pts.join(' ')+'" fill="none" stroke="'+(s.net_pnl>=0?'#047857':'#ef4444')+'" stroke-width="0.8"/>';
// Fill
h+='<polygon points="0,'+zeroY.toFixed(1)+' '+pts.join(' ')+' 100,'+zeroY.toFixed(1)+'" fill="'+(s.net_pnl>=0?'rgba(10,124,66,.1)':'rgba(239,68,68,.1)')+'"/>';
h+='</svg>';
h+='<div style="position:absolute;top:4px;left:8px;font-size:8px;color:var(--text3)">'+S+eqMax.toLocaleString('en-IN')+'</div>';
h+='<div style="position:absolute;bottom:4px;left:8px;font-size:8px;color:var(--text3)">'+S+eqMin.toLocaleString('en-IN')+'</div>';
h+='</div></div>';
}

// Result breakdown
if(d.result_breakdown){
h+='<div style="margin-bottom:14px"><div style="font-size:10px;font-weight:700;color:var(--text);margin-bottom:6px">Exit Type Breakdown</div>';
h+='<div style="display:flex;gap:6px;flex-wrap:wrap">';
var rb=d.result_breakdown;
var rbColors={'T3 HIT (FULL TARGET)':'#047857','SL AT COST (partial profit)':'#22d3ee','TIME EXIT (5D)':'#f59e0b','SL HIT':'#ef4444','BREAKEVEN':'#94a3b8','OPEN → FORCED CLOSE':'#8b5cf6'};
Object.keys(rb).forEach(function(k){
var c=rbColors[k]||'var(--text3)';
h+='<div style="padding:6px 12px;border-radius:6px;background:'+c+'10;border:1px solid '+c+'25;font-size:10px"><span style="font-weight:800;color:'+c+'">'+rb[k]+'</span> <span style="color:var(--text2)">'+k+'</span></div>';
});
h+='</div></div>';
}

// Monthly P&L heatmap
if(d.monthly&&Object.keys(d.monthly).length>0){
h+='<details style="margin-bottom:14px"><summary style="cursor:pointer;font-size:10px;font-weight:700;color:var(--blue);padding:6px 0">&#128197; Monthly Breakdown ('+Object.keys(d.monthly).length+' months)</summary>';
h+='<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:4px;margin-top:8px">';
Object.keys(d.monthly).forEach(function(m){
var md=d.monthly[m];
var mc=md.pnl>=0?'#047857':'#ef4444';
var wr=md.trades>0?Math.round(md.wins/md.trades*100):0;
h+='<div style="padding:6px 8px;border-radius:6px;background:'+mc+'08;border:1px solid '+mc+'15;font-size:9px">';
h+='<div style="font-weight:700;color:var(--text)">'+m+'</div>';
h+='<div style="font-weight:800;color:'+mc+'">'+( md.pnl>=0?'+':'')+S+Math.abs(Math.round(md.pnl)).toLocaleString('en-IN')+'</div>';
h+='<div style="color:var(--text3)">'+md.trades+' trades &middot; '+wr+'% WR</div></div>';
});
h+='</div></details>';
}

// Best/worst trades
h+='<details style="margin-bottom:14px"><summary style="cursor:pointer;font-size:10px;font-weight:700;color:var(--blue);padding:6px 0">&#127942; Top 5 Best & Worst Trades</summary>';
h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px">';
h+='<div><div style="font-size:9px;font-weight:700;color:#047857;margin-bottom:4px">BEST TRADES</div>';
(d.best_trades||[]).forEach(function(t){
h+='<div style="padding:4px 8px;font-size:9px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between"><span>'+t.date+' ('+t.confluence+')</span><span style="font-weight:700;color:#047857">+'+S+Math.abs(Math.round(t.pnl)).toLocaleString('en-IN')+'</span></div>';
});
h+='</div><div><div style="font-size:9px;font-weight:700;color:#ef4444;margin-bottom:4px">WORST TRADES</div>';
(d.worst_trades||[]).forEach(function(t){
h+='<div style="padding:4px 8px;font-size:9px;border-bottom:1px solid var(--border);display:flex;justify-content:space-between"><span>'+t.date+' ('+t.confluence+')</span><span style="font-weight:700;color:#ef4444">'+S+Math.abs(Math.round(t.pnl)).toLocaleString('en-IN')+'</span></div>';
});
h+='</div></div></details>';

// Algo config
h+='<div style="padding:10px 14px;border-radius:8px;background:var(--bg2);border:1px solid var(--border);font-size:9px;color:var(--text3);line-height:1.7;margin-bottom:10px">';
h+='<strong style="color:var(--text)">Algo Config:</strong> Confluence ≥'+d.confluence_threshold+' &middot; SL: '+d.sl_method+' &middot; Targets: '+d.targets+' &middot; Exit: '+d.exit_strategy+' &middot; Lot: '+d.lot;
h+='</div>';

// Verdict
var grade=s.win_rate>=60&&s.profit_factor>=1.5&&s.sharpe_ratio>=1?'A':s.win_rate>=50&&s.profit_factor>=1.2?'B':s.win_rate>=45&&s.profit_factor>=1?'C':'D';
var gradeC=grade==='A'?'#047857':grade==='B'?'#3b82f6':grade==='C'?'#f59e0b':'#ef4444';
h+='<div style="padding:14px;border-radius:10px;background:'+gradeC+'08;border:2px solid '+gradeC+'30;text-align:center">';
h+='<div style="font-size:36px;font-weight:900;color:'+gradeC+'">Grade: '+grade+'</div>';
h+='<div style="font-size:11px;color:var(--text2);margin-top:4px">';
if(grade==='A')h+='Strong algo. '+s.win_rate+'% WR with '+s.profit_factor+'× profit factor. The confluence approach works on this instrument.';
else if(grade==='B')h+='Decent performance. Profitable but could benefit from tighter entry criteria or different ATR multiplier.';
else if(grade==='C')h+='Marginal. Barely profitable. Consider raising the confluence threshold or adding intraday filters.';
else h+='Algo underperforms on this instrument. The confluence approach may not suit '+d.symbol+'. Try different parameters or skip.';
h+='</div></div>';

// Disclaimer
h+='<div style="margin-top:10px;padding:8px 12px;border-radius:6px;background:rgba(245,158,11,.04);border:1px solid rgba(245,158,11,.1);font-size:8px;color:var(--text3);line-height:1.6">';
h+='<strong style="color:var(--amber)">&#9888;</strong> Past performance does not guarantee future results. Backtest uses daily OHLCV data (not intraday). Real slippage, commissions, and liquidity impact are not modeled. Use paper trading before risking real capital.';
h+='</div>';
...
if(!el)return;
el.innerHTML='<div style="text-align:center;padding:40px"><div style="font-size:28px;animation:spin 1s linear infinite">&#9203;</div><div style="color:var(--text3);margin-top:8px;font-size:11px">Loading live trade signals...</div></div>';
try{
...
// ═══════════════════════════════════════════════
// ═══ SMART TRADES REGION TOGGLE ═══
window._stRegion='IN';
function switchSTRegion(reg){
window._stRegion=reg;
...
try{
const r=await fetch('/api/index-trades',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email,force_refresh:!!forceRefresh,region:stReg})});
const data=await r.json();
...
let ni='';
d.news.forEach(n=>{
var nc=n.type==='bullish'?'#4ade80':n.type==='bearish'?'#ff6b6b':'rgba(255,255,255,.5)';
ni+=`<span class="ni" style="color:${nc};padding:0 16px;font-size:10px;white-space:nowrap">${n.text}</span>`;
});
